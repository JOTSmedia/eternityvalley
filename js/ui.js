...
export const UI = {
  ...
  _initMemorialSearch(doPlaceSearch) {
    ...
    const render = () => {
      ...
      if (q.length < 2) return this._closeSuggest();
      const hits = this._rankMemorials(q);
      const rows = [];
      if (hits.length) {
        rows.push(`<div class="ss-head">${icon('paw')} Companions</div>`);
        hits.forEach((m) => {
          rows.push(`
      ...
      rows.push(`<div class="ss-head">${icon('globe')} Places</div>`);
      rows.push(`
      ...
      if (!hits.length) {
        rows.splice(0, 0, `<div class="ss-empty">No companion by that name yet.</div>`);
      }
      ...
      box.querySelector('[data-place]').onclick = () => { this._closeSuggest(); doPlaceSearch(); };
    };

    let t = null;
    input.addEventListener('input', () => { clearTimeout(t); t = setTimeout(render, 110); });
  ...
  async mountEarth() {
    ...
    // No orbit framing here: the view from space is our own globe
    // now, and Google's map is only ever entered by descending to a
    // specific place.
  },
  ...
  async earthMemorialForm(pos) {
    ...
    this.modal(`
      ...
      <label>Their charity — where this memorial's giving goes</label>
      <select id="emCharity">
        <option value="">Let each giver choose</option>
        ${CHARITIES.map(c => `<option value="${c.id}" ${State.data.charity === c.id ? 'selected' : ''}>${c.name}</option>`).join('')}
      </select>

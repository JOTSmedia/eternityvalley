  ...
  async show2D() {
    if (!this.map) {
      if (window.UI?.map) this.map = window.UI.map;
        ...
        if (!this.map) {
          try {
            const { Map2D } = await import('./map2d.js');
      ...
      el.innerHTML = `
        <div class="gw-summary-card">
          <div class="gw-summary-icon">
            ${photoDataUrl ? `<img src="${photoDataUrl}" style="width:100%;height:100%;object-fit:cover;border-radius:50%">` : speciesIcon(sp, { size: 28 })}
          </div>

...
// ============================================================
import { DISTRICTS, SIZE_LABELS, PET_NAMES, SPECIES, EPITAPHS } from './terrain.js?v=8';
import { MEMBERSHIPS, PLOT_ITEMS, GIFTS, EARTH_PLOT, PHYSICAL_KEEPSAKES, SLOTS, HEADSTONE_STYLES, ITEM_DECOR, GIFT_DECOR, CHARITIES, GIFT_CHARITY_SHARE, charityName, fmtPrice } from './catalog.js?v=8';
import { CharityUI } from './charityui.js?v=8';
import { Tour } from './tour.js?v=8';
import { SPLITS, Campaigns, Ledger, fmt, charityById, toCents, split } from './charity.js?v=8';
import { Auth } from './auth.js?v=8';
import { State } from './state.js?v=8';
import { checkout } from './checkout.js?v=8';
import { IS_DEMO, HAS_MAPS3D, IS_ADMIN } from './config.js?v=8';
import { RBV, allMemorials, allActivity, timeAgo } from './social.js?v=8';
import { Motion } from './motion.js?v=8';
import { icon, speciesIcon, speciesKey, rainbowMark, SPECIES_LABELS } from './icons.js?v=8';
import { thumbImg, canRender as canRenderThumb, photoFor } from './thumbs.js?v=8';
import { Soundscape } from './soundscape.js?v=8';
import { checkAnniversaries, generateICS, ANNIVERSARY_GIFTS } from './anniversary.js?v=8';
...
export const UI = {
    ...
    if (!this.globe) {
      if (this._globePromise) {
        ...
        this._globePromise = (async () => {
          const { Globe } = await import('./globe.js?v=8');
  ...
  async show2D() {
    if (!this.map) {
      if (window.UI?.map) this.map = window.UI.map;
        ...
        if (!this.map) {
          try {
            const { Map2D } = await import('./map2d.js?v=8');
  ...
  async mountEarth() {
    if (!this.earth) {
      const { EarthView } = await import('./earth.js?v=8');
  ...
  partnerModal() {
    ...
    box.querySelector('#ptSubmit').onclick = async () => {
      ...
      try {
        const cfg = await import('./config.js?v=8');
        if (cfg.HAS_API) {

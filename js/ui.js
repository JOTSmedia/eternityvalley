...
// ============================================================
import { DISTRICTS, SIZE_LABELS, PET_NAMES, SPECIES, EPITAPHS } from './terrain.js?v=7';
import { MEMBERSHIPS, PLOT_ITEMS, GIFTS, EARTH_PLOT, PHYSICAL_KEEPSAKES, SLOTS, HEADSTONE_STYLES, ITEM_DECOR, GIFT_DECOR, CHARITIES, GIFT_CHARITY_SHARE, charityName, fmtPrice } from './catalog.js?v=7';
import { CharityUI } from './charityui.js?v=7';
import { Tour } from './tour.js?v=7';
import { SPLITS, Campaigns, Ledger, fmt, charityById, toCents, split } from './charity.js?v=7';
import { Auth } from './auth.js?v=7';
import { State } from './state.js?v=7';
import { checkout } from './checkout.js?v=7';
import { IS_DEMO, HAS_MAPS3D, IS_ADMIN } from './config.js?v=7';
import { RBV, allMemorials, allActivity, timeAgo } from './social.js?v=7';
import { Motion } from './motion.js?v=7';
import { icon, speciesIcon, speciesKey, rainbowMark, SPECIES_LABELS } from './icons.js?v=7';
import { thumbImg, canRender as canRenderThumb, photoFor } from './thumbs.js?v=7';
import { Soundscape } from './soundscape.js?v=7';
import { checkAnniversaries, generateICS, ANNIVERSARY_GIFTS } from './anniversary.js?v=7';
...
export const UI = {
    ...
    if (!this.globe) {
      if (this._globePromise) {
        ...
        this._globePromise = (async () => {
          const { Globe } = await import('./globe.js?v=7');
  ...
  async show2D() {
    if (!this.map) {
      if (window.UI?.map) this.map = window.UI.map;
        ...
        if (!this.map) {
          try {
            const { Map2D } = await import('./map2d.js?v=7');
  ...
  async mountEarth() {
    if (!this.earth) {
      const { EarthView } = await import('./earth.js?v=7');
  ...
  partnerModal() {
    ...
    box.querySelector('#ptSubmit').onclick = async () => {
      ...
      try {
        const cfg = await import('./config.js?v=7');
        if (cfg.HAS_API) {

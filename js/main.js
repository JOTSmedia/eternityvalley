...
// ============================================================
import { generatePlots, DISTRICTS, PET_NAMES } from './terrain.js?v=6';
import { World3D } from './world3d.js?v=5';
import { Map2D } from './map2d.js?v=6';
import { Auth } from './auth.js?v=6';
import { State } from './state.js?v=6';
import { UI } from './ui.js?v=6';
import { SEASON_STYLE, MOODS } from './ambience.js?v=6';
import { EarthView } from './earth.js?v=6';
import { Theme } from './theme.js?v=6';
import { Atmosphere } from './atmosphere.js?v=6';
import { soundscape, Soundscape } from './soundscapes.js?v=6';
import { Motion } from './motion.js?v=6';
import { hydrate as hydrateIcons } from './icons.js?v=6';
import { warmThumbs, photosReady } from './thumbs.js?v=6';
import { Tour } from './tour.js?v=6';
import { CharityUI } from './charityui.js?v=6';
import { warmAllTextures } from './materials.js?v=6';
...
import { SLOTS, ITEM_DECOR, GIFT_DECOR, GIFTS, CHARITIES } from './catalog.js?v=6';
  ...
  enterPromise = (async () => {
    try {
      ...
      try {
        ...
        if (memId) {
          const { allMemorials } = await import('./social.js?v=6');
        ...
        if (cmpId) {
          const { Campaigns } = await import('./charity.js?v=6');
          const c = await Campaigns.get(cmpId);
          if (c) setTimeout(() => CharityUI.campaignModal(UI, c), 1200);
        }
        const chId = params.get('charity');
        if (chId) {
          const { charityById } = await import('./charity.js?v=6');
        ...
        if (ref) {
          ...
          const cfg = await import('./config.js?v=6');
          if (cfg.HAS_API) {
            let consent = false;
...
async function boot() { 
  ...
  document.getElementById('enterBtn')?.addEventListener('click', () => { Soundscape.init(); enter('tour'); });
  document.getElementById('sanctuaryEntryBtn')?.addEventListener('click', () => { Soundscape.init(); enter('tour'); });
  document.getElementById('enterValleyBtn')?.addEventListener('click', () => { Soundscape.init(); UI.show3D('tour'); });
  document.getElementById('btn3d')?.addEventListener('click', () => { Soundscape.init(); UI.show3D('orbit'); });
  document.getElementById('btnGlobe')?.addEventListener('click', () => { Soundscape.init(); UI.showGlobe(); });

...
// ============================================================
import { getDayPhase, getSeason, SEASON_STYLE, MOODS } from './ambience.js';
...
function nextPhase(key) { return ORDER[(ORDER.indexOf(key) + 1) % ORDER.length]; }
const BLEND = 0.12;

function blendedPalette(phase) {
  const cur = PALETTES[phase.key];
  if (phase.t < 1 - BLEND) return { pal: cur, blendT: 0, nextPal: cur };
  const nxt = PALETTES[nextPhase(phase.key)];

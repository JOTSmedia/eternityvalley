...
import { WORLD, DISTRICTS, ROADS, RIVER, RIVER_INLET, RIVER_OUTLET, terrainHeight, backgroundMountainElevation, distToRoads, distToRiver, getRiverInfo, riverWaterElevation, fbm, ridgeNoise, mulberry32, SIZE_DIMS } from './terrain.js?v=5';
import { getSeason, SEASON_STYLE, getDayPhase, PHASES, MOODS, fetchWeather } from './ambience.js?v=6';
import { Surfaces, waterNormalTexture, textures, material, createBotanicalFoliageMaterial, clearCache } from './materials.js?v=5';
...
import { WorldAssetLoader } from './WorldAssetLoader.js?v=6';
import { WorldLighting } from './WorldLighting.js?v=5';
import { WorldTerrain } from './WorldTerrain.js?v=5';
import { WorldTourController } from './WorldTourController.js?v=5';
// ============================================================
...
function createContactShadow(radius, yPos = 0.04) {
  if (!_sharedShadowMat) {
    ...
    _sharedShadowMat = new THREE.MeshBasicMaterial({ map: shadowTex, transparent: true, depthWrite: false, polygonOffset: true, polygonOffsetFactor: -4, polygonOffsetUnits: -4 });
  }
  if (!_sharedShadowGeo) {
...
export class World3DCore {
  ...
  _setupCameraBubbles() {
    ...
    const data = new Float32Array(bubbleCount * 3); // speed, phase, size
    
    for(let i = 0; i < bubbleCount; i++) {
      pos[i*3] = (Math.random() - 0.5) * 40;
      pos[i*3+1] = (Math.random() - 0.5) * 40;
      pos[i*3+2] = -(Math.random() * 40 + 5); // 5 to 45 units in front of camera
      
      data[i*3] = 0.5 + Math.random() * 1.5; // speed
      data[i*3+1] = Math.random() * Math.PI * 2; // phase
      data[i*3+2] = 0.1 + Math.random() * 0.4; // size
    }
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        void main() {
          vec3 p = position;
          // Float upwards relative to camera, but wrap around
          p.y += mod(uTime * aData.x + aData.y, 40.0) - 20.0;
          // Drift sideways
          p.x += sin(uTime * 0.5 + aData.y) * 2.0;
          
          vec4 mvPosition = modelViewMatrix * vec4(p, 1.0);
          gl_Position = projectionMatrix * mvPosition;
          
          gl_PointSize = (100.0 / -mvPosition.z) * aData.z;
          vAlpha = smoothstep(40.0, 10.0, -p.y) * smoothstep(-40.0, -30.0, p.z); // Fade near edges
        }
      `,
      fragmentShader: `
        ...
        void main() {
          ...
          // Ring effect for bubbles
          float ring = smoothstep(0.5, 0.4, dist) * smoothstep(0.3, 0.4, dist);
          float glow = smoothstep(0.5, 0.0, dist) * 0.3;
          
          gl_FragColor = vec4(uColor, (ring + glow) * uOpacity * vAlpha * 0.6);
        }
      `
    });
    
    this._bubbleMat = mat;
    this._bubblePoints = new THREE.Points(geo, mat);
    this._bubblePoints.frustumCulled = false;
    
    // Attach directly to camera so they always surround the viewer
    if (this.camera) {
       this.camera.add(this._bubblePoints);
    }
  }

  _setupUnderwaterOverlay() {
    if (typeof document === 'undefined') return;
    this._underwaterOverlay = document.createElement('div');
    this._underwaterOverlay.id = 'underwater-overlay';
    Object.assign(this._underwaterOverlay.style, {
      position: 'absolute',
      ...
      background: 'radial-gradient(circle at 50% 50%, rgba(24, 120, 168, 0.1) 0%, rgba(18, 82, 124, 0.4) 60%, rgba(0, 20, 50, 0.85) 100%)',
      boxShadow: 'inset 0 0 150px rgba(0,20,50, 0.95)',
      backdropFilter: 'blur(3px) contrast(1.1) brightness(0.9) hue-rotate(10deg)',
      webkitBackdropFilter: 'blur(3px) contrast(1.1) brightness(0.9) hue-rotate(10deg)',
      mixBlendMode: 'overlay'
    });
    
    // Water distortion ripple animation using CSS
    const style = document.createElement('style');
    style.innerHTML = `
      @keyframes underwaterPulse {
        0% { backdrop-filter: blur(2px) contrast(1.1) brightness(0.9) hue-rotate(10deg) saturate(1.2); transform: scale(1.0); }
        50% { backdrop-filter: blur(4px) contrast(1.15) brightness(1.1) hue-rotate(15deg) saturate(1.4); transform: scale(1.02); }
        100% { backdrop-filter: blur(2px) contrast(1.1) brightness(0.9) hue-rotate(10deg) saturate(1.2); transform: scale(1.0); }
      }
      .is-underwater-fx {
        animation: underwaterPulse 8s infinite ease-in-out;
      }
    `;
    document.head.appendChild(style);
    
    // Caustics overlay image
    this._causticsOverlay = document.createElement('div');
    Object.assign(this._causticsOverlay.style, {
      position: 'absolute',
      top: '0', left: '0', width: '100%', height: '100%',
      pointerEvents: 'none',
      zIndex: '16',
      opacity: '0',
      transition: 'opacity 0.5s ease-in-out',
      backgroundImage: 'url("https://raw.githubusercontent.com/mrdoob/three.js/master/examples/textures/waternormals.jpg")',
      backgroundSize: 'cover',
      mixBlendMode: 'color-dodge',
      filter: 'contrast(1.5) opacity(0.15)'
    });
    
    document.body.appendChild(this._underwaterOverlay);
    document.body.appendChild(this._causticsOverlay);
  }
  ...
  _init() {
    ...
    const renderer = new THREE.WebGLRenderer({
      ...
      logarithmicDepthBuffer: false, // Enables precise depth buffer across 0.1m - 50,000m scene
    });
    ...
    const maxDpr = isMobileDevice ? 1.5 : 1.0;
    ...
    this.camera = cam; this.scene.add(this.camera); console.log("[World3D] camera created");
  ...
  async initAsync() {
    ...
    const safe = async (name, fn) => {
      ...
      try {
        ...
        console.log('[world3d] initAsync done', name, elapsed + 'ms');
        if (elapsed > 1500) {
          console.warn(`[world3d] Stage ${name} exceeded 1500ms budget (${elapsed}ms). Optimize or chunk this builder.`);
        }
      }
    };
    ...
    // --- SYNCHRONOUS BUILD PHASE ---
    let p = 15;
    const step = 85 / 30; // 30 builders
    const nextProgress = () => progress(p += step);

    await safe('terrain', () => this.terrain._terrain()); nextProgress();
    await safe('water', () => this.terrain._water()); nextProgress();
    await safe('mountainWaterfall', () => this.terrain._mountainWaterfall()); nextProgress();
    await safe('rainbowBridge', () => this.terrain._rainbowBridge()); nextProgress();
    await safe('gate', () => this.terrain._gate()); nextProgress();
    await safe('proceduralPlots', () => this.terrain._generateProceduralPlots()); nextProgress();
    await safe('plots', () => this._plots()); nextProgress();
    await safe('backgroundMountains', () => this._backgroundMountains()); nextProgress();
    await safe('oceanWaterfall', () => this.terrain._oceanWaterfall()); nextProgress();
    await safe('coastalCliff', () => this.terrain._coastalCliff()); nextProgress();
    await safe('highlandSanctuary', () => this.terrain._highlandSanctuary()); nextProgress();
    await safe('universalCathedral', () => this.terrain._universalCathedral()); nextProgress();
    await safe('godRays', () => this.lighting._godRays()); nextProgress();
    await safe('roads', () => this.terrain._roads()); nextProgress();
    await safe('plaza', () => this.terrain._plaza()); nextProgress();
    await safe('pawprints', () => this.terrain._pawprints()); nextProgress();
    await safe('vegetation', () => this.terrain._vegetation()); nextProgress();
    await safe('meadowCarpet', () => this.terrain._meadowCarpet()); nextProgress();
    await safe('blooms', () => this.terrain._blooms()); nextProgress();
    await safe('districtFeatures', () => this.terrain._districtFeatures()); nextProgress();
    await safe('sanctuaryTree', () => this.terrain._sanctuaryTree()); nextProgress();
    await safe('riverLanterns', () => this.terrain._riverLanterns3D()); nextProgress();
    await safe('celestialMotes', () => this.terrain._celestialMotes()); nextProgress();
    await safe('underwaterWorld', () => this.terrain._underwaterWorld()); nextProgress();
    await safe('moorishMosque', () => this.terrain._moorishMosque()); nextProgress();
    await safe('buddhistPagoda', () => this.terrain._buddhistPagoda()); nextProgress();
    await safe('kayaIsland', () => this.terrain._kayaIsland()); nextProgress();
    await safe('picking', () => this._picking()); nextProgress();
    await safe('composer', () => this.lighting._composer()); nextProgress();
    await safe('initAmbienceControls', () => this._initAmbienceControls()); nextProgress();
    await safe('river', () => this.terrain._river()); nextProgress();

    console.log('[world3d] All builders complete. Releasing UI preloader.');
  ...
  async _backgroundMountains() {
    ...
    const SEG_THETA = 128, SEG_RADIAL = 48;
    ...
    for (let i = 0; i < pos.count; i++) {
      if (performance.now() - lastYield > 16) {
        ...
        if (lastYield - tStart > 1500) {
          console.warn('[world3d] _backgroundMountains exceeded 1500ms budget, aborting loop at index', i, '/', pos.count);
          break; // Abort the loop if it exceeds budget
        }
      }
...
//     matMossBoulder.color.setHex(0x4a5d4e);
  ...
  _picking() {
    ...
    this._onPointerMove = e => {
      ...
      if (pickingRaf) return;
      pickingRaf = requestAnimationFrame(() => {
        pickingRaf = null;
        const r = this.canvas.getBoundingClientRect();
        ptr.x = ((e.clientX - r.left) / r.width) * 2 - 1;
        ptr.y = -((e.clientY - r.top) / r.height) * 2 + 1;
  ...
  _updateAdaptivePerformance(dt) {
    ...
    if (medianMs > 16.6) {
       this._timeUnder60fps = (this._timeUnder60fps || 0) + dt;
       
       if (this._timeUnder60fps > 2.0) {
           this._timeUnder60fps = 0; // reset to avoid doing it every frame immediately
           
           // Aggressive shadow map reduction
           if (this.lighting && this.lighting.sun && this.lighting.sun.shadow && this.lighting.sun.shadow.mapSize.width > 256) {
               this.lighting.sun.shadow.mapSize.width = Math.max(256, this.lighting.sun.shadow.mapSize.width / 2);
               this.lighting.sun.shadow.mapSize.height = Math.max(256, this.lighting.sun.shadow.mapSize.height / 2);
           ...
           // Instanced mesh cull (heavy vegetation/grass) - skip if cinematic
           if (this.scene) {
               const isCinematic = this.tourController && (this.tourController.tourMode || this.tourController._entranceFlight);
               if (!isCinematic) {
                   this.scene.traverse((c) => {
                       if (c.isInstancedMesh && c.count > 50) {
                           c.count = Math.floor(c.count * 0.7);
                       }
                   });
               }
           }
    ...
    const minScale = isMobile ? 0.40 : 0.50; // Hardline minimum resolution scaling
    const maxScale = isMobile ? 0.75 : 1.0;
    let targetScale = this._renderScale;
    
    // User requested HARD LINE 60FPS (16.6ms frame time)
    if (medianMs > 16.8 && this._renderScale > minScale) {
       targetScale = Math.max(minScale, this._renderScale - 0.10); // aggressive downgrade
       
       // Force Culling Distance aggressively if under 60fps (unless in cinematic tour mode)
       const isCinematic = this.tourController && (this.tourController.tourMode || this.tourController._entranceFlight);
       if (!isCinematic && this.camera && this.camera.far > 2500) {
           this.camera.far = Math.max(2500, this.camera.far - 150);
           this.camera.updateProjectionMatrix();
       }
    } else if (medianMs < 15.0 && this._renderScale < maxScale) {
       // Gently scale up if safely above 60fps
       targetScale = Math.min(maxScale, this._renderScale + 0.02);
    }
  ...
  _optimizeScene() {
    ...
    this.scene.traverse((o) => {
        ...
        if (o.userData && (o.userData.speedX !== undefined || o.userData.phase !== undefined || o.userData.isDynamic)) return;
        if (o.name && (o.name.includes('Water') || o.name.includes('Sky') || o.name.includes('Cloud') || o.name.includes('Terrain') || o.name.includes('GateDoor') || o.name.includes('Fountain'))) return;
        if (o.material) {
  ...
  _animate() {
    ...
    if (this.terrain.leftGateDoor && this.terrain.rightGateDoor) {
      ...
      const camDistGate = WORLD.gate ? Math.hypot(this.camera.position.x - WORLD.gate.x, this.camera.position.z - WORLD.gate.z) : 99999;
      
      let shouldOpen = false;
      if (this.tourController.tourMode) {
        // Delay opening until the drone is approaching the gate (Z=880) so the user sees the majestic swing
        shouldOpen = (camZ < 1050) || this.terrain._forceGateOpen;
        ...
        shouldOpen = (this.tourController.walkPos.z < 960) || (camDistGate < 450) || this.terrain._forceGateOpen;
      } else {
      ...
      const hingeSpeed = shouldOpen ? 6.5 : 2.0;
    ...
    // Fish animation loop has been delegated entirely to WorldTerrain._updateUnderwater(dt, t) by LOKI Architecture Audit.
    ...
    // 1. Highland Glacial Tarn Water Basin (High-altitude crystal lake at y=182.0m, deep basin floor y=158m, z=-615..-735, |x| < 60)
    const isUnderTarn = (Math.abs(camX) < 62 && camZ <= -615 && camZ >= -735) && camY < 182.2;

    // 2. Waterfall Plunge Pool (y=14.5m, bed at 4.5m)
    const dPlunge = Math.hypot(camX, camZ - (-550));
    const isUnderPlunge = dPlunge < 58.0 && camY < 14.7;
    ...
    const isUnderLake = (WORLD.lake && Math.hypot(camX - WORLD.lake.x, camZ - WORLD.lake.z) < (WORLD.lake.r + 5.0) && camY < (WORLD.waterLevel + 0.2)) ||
    ...
    let isUnderwater = isUnderTarn || isUnderPlunge || isUnderLake || isUnderOcean;
    // Catch-all: if we drop below general water level everywhere else
    if (!isUnderwater && camY < (WORLD.waterLevel || 12.5) && camZ <= 915 && camZ > -400) {
      isUnderwater = true;
    }
    ...
    if (isUnderOcean) {
      ...
      const plungeDepth = Math.max(0.0, 14.5 - camY);
    ...
    if (this._godRaysMat) {
      this._godRaysMat.uniforms.uTime.value = t;
      this._godRaysMat.uniforms.uOpacity.value = this._underwaterBlend;
    }
    if (this._underwaterOverlay) {
      this._underwaterOverlay.style.opacity = this._underwaterBlend;
      this._causticsOverlay.style.opacity = this._underwaterBlend;
      if (this._underwaterBlend > 0.01 && !this._underwaterOverlay.classList.contains('is-underwater-fx')) {
         this._underwaterOverlay.classList.add('is-underwater-fx');
      } else if (this._underwaterBlend < 0.01 && this._underwaterOverlay.classList.contains('is-underwater-fx')) {
         this._underwaterOverlay.classList.remove('is-underwater-fx');
      }
      // Parallax caustics movement based on camera rotation/time
      this._causticsOverlay.style.backgroundPosition = `${Math.sin(t * 0.2) * 50}px ${Math.cos(t * 0.3) * 50}px`;
    }
    ...
    if (this.scene.fog) {
      ...
      console.log('[world3d] _animate error in frame:', err.stack ? err.stack : err.message || err);
      try {
  ...
  // Wrapper APIs for tour and camera modes (required for UI/main.js)
  setMode(mode) {
    if (this.tourController) this.tourController.setMode(mode);
  }

  startDroneTour(i = 0) {
    if (this.tourController) this.tourController.startDroneTour(i);
  }
  startEntranceFlight(opts) { return this.tourController?.startEntranceFlight(opts); }

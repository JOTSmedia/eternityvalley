  ...
  _updateAdaptivePerformance(dt) {
    ...
    if (medianMs > 16.6) {
       ...
       if (this._timeUnder60fps > 2.0) {
           ...
           // Instanced mesh cull (heavy vegetation/grass)
           if (this.scene) {
               this.scene.traverse((c) => {
    ...
    if (medianMs > 16.8 && this._renderScale > minScale) {
       ...
       // Force Culling Distance aggressively if under 60fps
       if (this.camera && this.camera.far > 800) {
           this.camera.far = Math.max(800, this.camera.far - 150);
           this.camera.updateProjectionMatrix();
       }

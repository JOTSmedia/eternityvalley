...
const isHigh = typeof window !== 'undefined' && window.innerWidth > 768 ? 1024 : 512;
  ...
  async _water() {
    ...
    this.riverMat = this._createPhysicalWaterMaterial(normals, 'river');
    ...
    const troutSchools = [
      // 1. Highland Glacial Tarn (New Plunge Pool, waterLevel = 14.5m, deep bed at -9.5m -> 24m deep!)
      { cx: 0,   cy: -6.0,  cz: -570, radiusX: 38, radiusZ: 32, count: 28, type: 'abyssal_trout' },  // Deep Submerged Granite Floor (20.5m below surface)
      { cx: 16,  cy: -2.5,  cz: -550, radiusX: 34, radiusZ: 36, count: 28, type: 'abyssal_trout' },  // Deep Basin Gliders (17m below surface)
      { cx: -18, cy: 1.5,   cz: -575, radiusX: 32, radiusZ: 30, count: 28, type: 'glacial_trout' },  // Mid-depth Emerald Alpine Brook Trout (13m below surface)
      { cx: 8,   cy: 5.0,   cz: -530, radiusX: 32, radiusZ: 34, count: 26, type: 'glacial_trout' },  // Mid-depth North Channel Brook Trout (9.5m below surface)
      { cx: -15, cy: 8.0,   cz: -555, radiusX: 30, radiusZ: 28, count: 24, type: 'sapphire_gliders' }, // West Cathedral Shoreline & Cavern Gliders (6.5m below surface)
      { cx: 12,  cy: 9.5,   cz: -580, radiusX: 28, radiusZ: 26, count: 22, type: 'sapphire_gliders' }, // East Cathedral Submerged Shelf (5m below surface)
      { cx: 0,   cy: 12.0,  cz: -510, radiusX: 26, radiusZ: 26, count: 22, type: 'rapids_trout' },   // Shallow Waterfall Outflow Lip & Rapids (2.5m below surface)

      // 2. Mid-River Grotto (Old Base of Waterfall, waterLevel = 13.5m, deep bed at 0.0m -> 13.5m deep!)
      { cx: 0,   cy: 3.0,   cz: -362, radiusX: 34, radiusZ: 34, count: 26, type: 'grotto_trout' },   // Deep Churning Grotto Basin (10.5m below surface)
      { cx: 16,  cy: 7.0,   cz: -355, radiusX: 30, radiusZ: 32, count: 24, type: 'grotto_trout' },   // Mid-Grotto Basin (6.5m below surface)
      { cx: -14, cy: 10.0,  cz: -365, radiusX: 28, radiusZ: 30, count: 22, type: 'cascade_gliders' }, // Cascade Mist Swirlers (3.5m below surface)

      // 3. Rainbow River Canyon & Bridges (waterLevel = 14.5 -> 12.5 -> 7.8m)
      { cx: 75,  cy: 10.4,  cz: -345, radiusX: 32, radiusZ: 28, count: 24, type: 'river_gliders' },  // Inlet River Gorge
...
//     wetRockMat.roughness = 0.25;
    ...
    const waterfallShader = {
      ...
      vertexShader: `
        ...
        void main() {
          ...
          // Snoqualmie Falls sheer drop velocity and turbulence
          float speedMulti = 1.5;
          vec3 g1 = gerstnerWave(vec2(0.0, 1.0), 0.3, 4.0, pos.xy, 6.0 * speedMulti, uTime);
          vec3 g2 = gerstnerWave(vec2(0.2, 0.9), 0.25, 2.5, pos.xy, 8.0 * speedMulti, uTime);
          vec3 g3 = gerstnerWave(vec2(-0.2, 0.9), 0.15, 1.5, pos.xy, 9.0 * speedMulti, uTime);
          
          vec3 gWave = (g1 + g2 + g3) * smoothstep(0.02, 0.15, uv.y) * smoothstep(1.0, 0.95, uv.y);
          pos += gWave;
      ...
      fragmentShader: `
        ...
        void main() {
          // Multi-frequency FBM foam aeration for roaring white water
          vec2 flow1 = vec2(vUv.x * 16.0, vUv.y * 30.0 - uTime * 8.5);
          vec2 flow2 = vec2(vUv.x * 32.0 + 3.0, vUv.y * 45.0 - uTime * 12.2);
          vec2 spray = vec2(vUv.x * 65.0, vUv.y * 85.0 - uTime * 18.5);

          float n1 = fbm(flow1);
          float n2 = fbm(flow2);
          float nSpray = noise(spray);

          // Roaring white water mostly foam with deep crystalline cuts
          float foamMask = smoothstep(0.25, 0.85, n1 * 0.5 + n2 * 0.35 + nSpray * 0.25);
          
          // Edge aeration (turbulent friction against sheer basalt bedrock)
          float edgeDist = abs(vUv.x - 0.5) * 2.0;
          float edgeFoam = smoothstep(0.55, 0.95, edgeDist) * 0.65;
          foamMask = clamp(foamMask + edgeFoam + nSpray * 0.4, 0.0, 1.0); // extra mist/spray

          // Crystalline glacial depth gradient (Snoqualmie falls water can be dark/emerald)
          vec3 waterCol = mix(uDeepColor, uGlacierColor, n1 * 0.8 + 0.2);
          vec3 finalCol = mix(waterCol, uFoamColor, foamMask);

          // Specular highlights for violent spray catching the sun
          vec3 viewDir = normalize(cameraPosition - vWorldPos + vec3(0.0001));
          vec3 halfVector = normalize(vec3(0.5, 0.8, 0.3) + viewDir + vec3(0.0001));
          float spec = pow(max(0.0, dot(vNormal, halfVector)), 24.0) * 0.9;
          finalCol += vec3(spec);

          // Thicker mist at the bottom for roaring impact
          float mistAlpha = smoothstep(0.3, 0.0, vUv.y) * 0.8 * nSpray;
          float alpha = mix(0.90, 1.0, foamMask) * smoothstep(0.0, 0.02, vUv.y) * smoothstep(1.0, 0.98, vUv.y);
          alpha = clamp(alpha + mistAlpha, 0.0, 1.0);
          alpha *= smoothstep(1.0, 0.9, edgeDist);
  ...
  // upperTarnMesh removed, plateau is solid
    ...
    // and plunging 157m vertically down the mountain face into the plunge pool at (z = -550, y = 14.5m)
    const fallPoints = [
      new V3(0, 182.0, -615),   // Cathedral plaza outflow
      new V3(0, 182.0, -610),   // Cliff edge
      new V3(0, 138.0, -610),   // Free vertical fall begins
      new V3(0, 83.9, -600),    // Mid-cataract roaring spray
      new V3(0, 42.0, -580),    // Lower cataract churning curtain
      new V3(0, 24.0, -565),    // Base impact cascade
    ...
    // 4. Physical Plunge Pool Water Basin (Disc of water with foam churning at z=-550)
...
#include <logdepthbuf_vertex>
  ...
  async _vegetation() {
    ...
    const place = (x, z) => {
      ...
      if (Math.hypot(x, z - (-550)) < 68 && h < 19.0) return null; // Keep plunge pool clear of trees
...
//     if (palmBark && palmBark.color) palmBark.color.setHex(0xa88e6e);
    ...
    (function(targetGroup) {
        ...
        const _gargoyleMat = typeof material === 'function' ? material('rockCliff', { repeat: 3.0, color: 0x333333, roughness: 0.8, metalness: 0.1, normalScale: 3.0 }) : new THREE.MeshStandardMaterial({color: 0x333333, roughness: 0.8});
  ...
  _universalCathedral() {
    ...
    // MAGNIFICENT UPGRADES - Buttresses, Stonework, Statues, Gargoyles
    (function(targetGroup) {
        const _weatheredStone = material('agedCaenLimestone', { repeat: 16.0, color: 0x6e6556, roughness: 0.95, metalness: 0.05, normalScale: 2.5, aoMapIntensity: 2.0 });
        const _statueMat = material('honedCarraraMarble', { repeat: 2.0, color: 0xe0e0e0, roughness: 0.3, metalness: 0.1 });
        const _gargoyleMat = material('rockCliff', { repeat: 3.0, color: 0x333333, roughness: 0.8, metalness: 0.1, normalScale: 3.0 });
    ...
    // Master Material Palette - Rugged basalt ashlar for sheer drops and majestic look
    const stone = Surfaces.agedCaenLimestone(14.0);
    // Deep sheer basalt dark stone for structural foundation and dramatic cliff contrast
    const darkStone = material('agedCaenLimestone', { repeat: 12.0, color: 0x2c3238, roughness: 0.92, metalness: 0.05, normalScale: 3.5, aoMapIntensity: 2.5 });
...
//     soilMat.color.setHex(0x3e3224);
    ...
    // 6b. Inner Portcullis & Guard Chambers
    const portcullisGroup = new THREE.Group();
    portcullisGroup.position.set(0, 0, -8.0); // 8m behind main gates

    // Heavy Stone Reinforcing Pillars
    const pillarGeos = [];
    for (let side of [-1, 1]) {
      const px = side * (HALF - 2.0);
      const pillarBase = new THREE.BoxGeometry(4.0, 2.0, 4.0);
      pillarBase.translate(px, 1.0, 0);
      pillarGeos.push(pillarBase);

      const pillarShaft = new THREE.BoxGeometry(3.0, 36.0, 3.0);
      pillarShaft.translate(px, 20.0, 0);
      pillarGeos.push(pillarShaft);
    }
    const mergedPillars = safeMerge(pillarGeos, false);
    if (mergedPillars) {
      const pillarMesh = new THREE.Mesh(mergedPillars, stoneLight);
      pillarMesh.castShadow = true;
      portcullisGroup.add(pillarMesh);
    }

    // Inner Portcullis Grille (Iron)
    const portcullisGeos = [];
    const pW = HALF * 2 - 4.0;
    const pH = 35.0;
    for (let x = -pW / 2 + 1; x < pW / 2; x += 2.0) {
      const vBar = new THREE.CylinderGeometry(0.2, 0.2, pH, 8);
      vBar.translate(x, pH / 2 + 1.0, 0);
      portcullisGeos.push(vBar);
      
      const spike = new THREE.ConeGeometry(0.3, 1.0, 4);
      spike.translate(x, 1.0 - 0.5, 0);
      spike.rotateX(Math.PI); // Point down
      portcullisGeos.push(spike);
    }
    for (let y = 3.0; y < pH; y += 2.5) {
      const hBar = new THREE.BoxGeometry(pW, 0.3, 0.3);
      hBar.translate(0, y, 0);
      portcullisGeos.push(hBar);
    }
    const mergedPortcullis = safeMerge(portcullisGeos, false);
    if (mergedPortcullis) {
      const portcullisMesh = new THREE.Mesh(mergedPortcullis, darkBronze);
      portcullisMesh.castShadow = true;
      portcullisGroup.add(portcullisMesh);
    }

    // Interior Guard Chambers built into the reinforcing pillars
    for (let side of [-1, 1]) {
      const px = side * (HALF - 2.0);
      const chamber = new THREE.Group();
      chamber.position.set(px, 0, -3.0);

      const cBase = new THREE.Mesh(new THREE.BoxGeometry(5.0, 8.0, 5.0), stoneLight);
      cBase.position.set(0, 4.0, 0);
      chamber.add(cBase);

      const cRoof = new THREE.Mesh(new THREE.ConeGeometry(4.0, 3.0, 4), marble);
      cRoof.rotation.y = Math.PI / 4;
      cRoof.position.set(0, 9.5, 0);
      chamber.add(cRoof);

      // Arrow slits
      const slit = new THREE.Mesh(new THREE.BoxGeometry(0.4, 2.0, 0.8), darkBronze);
      slit.position.set(side * 1.5, 4.5, 2.5);
      chamber.add(slit);

      // Torches illuminating the chambers
      const torch = new THREE.Group();
      torch.position.set(side * 2.0, 5.0, 2.7);
      
      const torchHandle = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.05, 0.8, 8), darkBronze);
      torchHandle.rotation.x = Math.PI / 6;
      torch.add(torchHandle);
      
      const torchFlame = new THREE.Mesh(new THREE.ConeGeometry(0.3, 0.8, 6), new THREE.MeshBasicMaterial({ color: 0xffaa00 }));
      torchFlame.position.set(0, 0.6, 0.2);
      torch.add(torchFlame);
      
      chamber.add(torch);
      portcullisGroup.add(chamber);
    }
    g.add(portcullisGroup);

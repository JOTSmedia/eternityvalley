...
//     matMossBoulder.color.setHex(0x4a5d4e);
  ...
  _updateWalk(dt) {
    ...
    if (this.tourMode) {
      ...
      const hT = 0.0004;
      if (!this._tourSpline) this._initTourSpline();
      const dA = this._tourSpline.getPoint(((normT - hT) % 1 + 1) % 1);
      const dB = this._tourSpline.getPoint((normT + hT) % 1);
      const metresPerT = Math.max(1e-3, dA.distanceTo(dB) / (2 * hT));
      if (!this._tourPaused) this._tourTime += safeDt * (targetMps / metresPerT) * this._tourSpeed * speedMult;

  ...
  _createPhysicalWaterMaterial(normals, type = 'lake') {
    ...
    mat.onBeforeCompile = (shader) => {
      ...
      shader.vertexShader = shader.vertexShader.replace(
        ...
        `#include <begin_vertex>
         vec4 worldPos = modelMatrix * vec4(position, 1.0);
         float w1 = sin(worldPos.x * 0.85 + worldPos.z * 0.52 + uTime * 1.15) * 0.18;
         float w2 = sin(worldPos.x * -0.48 + worldPos.z * 0.88 + uTime * 1.45) * 0.14;
         float w3 = sin(worldPos.x * 0.32 + worldPos.z * -0.95 + uTime * 1.85) * 0.10;
         float globalWave = w1 + w2 + w3;

         ${type === 'river' ? `
         float channelProfile = 1.0 - pow(abs(uv.x - 0.5) * 2.0, 2.0);
         float currentSpeed = 4.8 + channelProfile * 3.6;
         float rw1 = sin(uv.y * 3.8 - uTime * currentSpeed) * 0.16 * channelProfile;
         float rw2 = cos(uv.y * 7.5 + uv.x * 3.14159 - uTime * (currentSpeed * 1.35)) * 0.09 * channelProfile;
         float rw3 = sin(uv.y * 15.0 - uTime * (currentSpeed * 1.9)) * 0.04;
         float riverWave = rw1 + rw2 + rw3;
         ...
         transformed.y += mix(globalWave, riverWave, blend);
         ` : `
         transformed.y += globalWave;
         `}
        `
      );
    };
...
//     matMossBoulder.color.setHex(0x4a5d4e);
  ...
  _updateWalk(dt) {
    ...
    if (this.tourMode) {
      // this.gateTargetOpen = 1.0; // Let proximity check handle this
  ...
  _animate() {
    ...
    if (this._gateLeft && this._gateRight) {
      ...
      this._gateLeft.rotation.y = this.gateOpenAmount * -1.85;
      // Swing open inward toward the valley (PI + 1.48 rad)
      this._gateRight.rotation.y = Math.PI + this.gateOpenAmount * 1.85;
    }
    if (this.water) {

...
const isHigh = typeof window !== 'undefined' && window.innerWidth > 768 ? 1024 : 512;
  ...
  async _water() {
    ...
    const turtleData = [
      { cx: 15,  cy: -8.5, cz: 2225, radiusX: 35, radiusZ: 40, speed: 0.34, orbitSpeed: 0.046, scale: 2.6, phase: 0.3, dir: 1, color: [0.24, 0.46, 0.20] }, // Gliding at dive plunge entry
      { cx: -18, cy: -14.0, cz: 2275, radiusX: 42, radiusZ: 48, speed: 0.31, orbitSpeed: 0.042, scale: 2.7, phase: 1.6, dir: -1, color: [0.22, 0.44, 0.18] }, // Soaring along coral wall
      { cx: -35, cy: -18.5, cz: 2245, radiusX: 38, radiusZ: 42, speed: 0.33, orbitSpeed: 0.048, scale: 2.5, phase: 3.0, dir: 1, color: [0.26, 0.48, 0.22] }, // Mid-trench gliding
      { cx: 38,  cy: -12.0, cz: 2290, radiusX: 45, radiusZ: 50, speed: 0.28, orbitSpeed: 0.038, scale: 2.8, phase: 4.4, dir: -1, color: [0.20, 0.40, 0.16] }, // East coral garden
      { cx: -42, cy: -22.0, cz: 2330, radiusX: 52, radiusZ: 56, speed: 0.30, orbitSpeed: 0.040, scale: 2.6, phase: 2.1, dir: 1, color: [0.25, 0.45, 0.19] }, // Deep shelf channel
      { cx: 0,   cy: -26.0, cz: 2360, radiusX: 58, radiusZ: 64, speed: 0.27, orbitSpeed: 0.036, scale: 2.9, phase: 5.2, dir: -1, color: [0.23, 0.47, 0.21] }, // Deep lagoon trench
      { cx: -45, cy: -6.5, cz: 1150, radiusX: 55, radiusZ: 65, speed: 0.33, orbitSpeed: 0.044, scale: 2.5, phase: 1.2, dir: 1, color: [0.22, 0.42, 0.18] },  // Coastal ocean reef
      { cx: 55,  cy: -7.5, cz: 1220, radiusX: 60, radiusZ: 70, speed: 0.29, orbitSpeed: 0.038, scale: 2.6, phase: 3.7, dir: -1, color: [0.24, 0.46, 0.20] },  // Coastal ocean reef
    ];
...
//     matMossBoulder.color.setHex(0x4a5d4e);
  ...
  _initTourSpline() {
    ...
    const tourPoints = [
      ...
      new THREE.Vector3(0, 75.0, 1200),   // 0: Panoramic establishing aerial view
      new THREE.Vector3(0, 60.0, 1040),   // 1: Lining up with the grand approach
      new THREE.Vector3(0, 52.0, 980),    // 2: Monumental approach on Grand Marble Esplanade
      new THREE.Vector3(0, 51.0, 930),    // 3: Gliding between classical torchiere colonnades
      new THREE.Vector3(0, 50.0, 880),    // 4: Flight straight through triumphal archway center
      new THREE.Vector3(0, 48.0, 820),    // 5: Passing inner gate colonnade
      new THREE.Vector3(0, 42.0, 700),    // 6: Gliding down imperial avenue
      new THREE.Vector3(0, 38.5, 560),    // 7: Aligning with Rainbow Bridge south approach
      ...
      new THREE.Vector3(0, 172.0, -492),  // 32: Submerged dive into 24m deep Glacial Tarn (10m below surface)
      new THREE.Vector3(12, 169.5, -512), // 33: Deep submerged tarn basin glide over granite floor
      new THREE.Vector3(14, 171.0, -528), // 34: Swimming through crystal depths with celestial brook trout
      new THREE.Vector3(0, 173.5, -540),  // 35: Cruising deep alpine tarn, curving upward
      new THREE.Vector3(-10, 180.5, -548),// 36: Approaching water surface
      new THREE.Vector3(-6, 196.0, -554), // 37: Clean breach breaking water surface
      new THREE.Vector3(0, 224.0, -565),  // 38: Majestic vertical rocket climb high above Cathedral terrace
      new THREE.Vector3(45, 245.0, -590), // 39: Soaring high into cathedral aerial orbit approach
      ...
      new THREE.Vector3(-150, 206.0, -520),// 48: High panoramic descent along western mountain ridge
      new THREE.Vector3(-340, 172.0, -410),// 49: Gliding along western mountain ridge
      new THREE.Vector3(-430, 138.0, -310),// 50: Approach toward turquoise ribbed dome
      ...
      new THREE.Vector3(440, 5.0, -280),   // 61: Hydrodynamic dive into 14.5m deep lake basin
      new THREE.Vector3(465, 5.5, -325),   // 62: Cruising underwater with Golden Koi & Emperors
      new THREE.Vector3(390, 6.0, -330),   // 63: Submerged turn through dancing sunlit caustics
      ...
      new THREE.Vector3(15, -14.0, 2270),  // 79: 40m oceanic coral abyss plunge at y = -14.0m

      // === LEG 11: CELESTIAL SUNRISE ASCENT (Stage 11: t in [0.909, 1.000], indices 80..87) ===
      new THREE.Vector3(-45, -16.0, 2280), // 80: Submarine abyss glide among manta rays & sharks
      new THREE.Vector3(-110, 32.0, 2140), // 81: Sunrise breach in western open sea into golden sunlight
      new THREE.Vector3(-160, 85.0, 1820), // 82: Soaring climb in open sky
      new THREE.Vector3(-200, 140.0, 1520),// 83: Sweeping high altitude climb
  ...
  _calculateTourLookTarget(t, pos, tangent, outTarget) {
    ...
    if (normT < 10 * S) {
      if (normT < 9 * S + S * 0.4) {
        ...
        target.set(20.0 + (15.0 - 20.0) * frac, 36.5 + (-14.0 - 36.5) * frac, 2148.0 + (2240.0 - 2148.0) * frac);
      } else {
        const frac = smooth((normT - (9 * S + S * 0.85)) / (S * 0.15));
        target.set(15.0 + (-25.0 - 15.0) * frac, -14.0 + (-16.0 - -14.0) * frac, 2240.0 + (2280.0 - 2240.0) * frac);
      }
      return target;
    }
  ...
  startEntranceFlight(opts = {}) {
    ...
    const pathPoints = [
      new THREE.Vector3(0, 75.0, 980),  // Outside Grand Gate establishing monumental framing in open air
      new THREE.Vector3(0, 60.0, 930),  // Gliding down the grand approach avenue between torchiere columns
      new THREE.Vector3(0, 50.0, 880),  // Passing directly through soaring upper triumphal arch threshold (y = 50m)
      new THREE.Vector3(0, 48.0, 780),  // Sweeping through inner colonnade portals onto sunlit Grand Boulevard
      new THREE.Vector3(0, 45.0, 650),  // Gliding along avenue with emerald valley panorama opening
  ...
  setMode(mode) {
    ...
    if (mode === 'walk') {
      ...
      this._currentRoll = 0.0;
      if (hint) hint.classList.remove('is-active');
      if (!this.controls.target || this.controls.target.lengthSq() < 1.0) {
        this.camera.position.set(0, 48.0, 960);
  ...
  _animate() {
    ...
    try {
      ...
      if (now - this._lastFpsHudUpdate >= 200 && this._fpsCount >= 5) {
        ...
        if (!this._drsInitialized) {
          ...
          this._drsPixelRatio = isMob ? 0.75 : Math.min(window.devicePixelRatio || 1.0, 1.0);
    ...
    if (this._gateLeft && this._gateRight) {
      ...
      if (this.tourMode) {
        ...
        shouldOpen = this.gateTargetOpen === 1.0 || (camZ < 950) || this._forceGateOpen;
    ...
    if (this._lakeShader?.userData?.shader?.uniforms?.uTime) {
      this._lakeShader.userData.shader.uniforms.uTime.value = t;
    } else if (this._lakeShader?.uniforms?.uTime) {
      this._lakeShader.uniforms.uTime.value = t;
    }
    if (this._oceanShader?.userData?.shader?.uniforms?.uTime) {
      this._oceanShader.userData.shader.uniforms.uTime.value = t;
    } else if (this._oceanShader?.uniforms?.uTime) {
      this._oceanShader.uniforms.uTime.value = t;
    }
    ...
    if (this._troutMesh && this._troutData) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        const wanderX = Math.sin(troutTime * 0.18 + f.phase) * (f.wanderAmp || 6.0);
        const wanderZ = Math.cos(troutTime * 0.14 + f.phase * 1.3) * (f.wanderAmp || 6.0);
        ...
        const safeCy = Math.min(waterSurface - 0.8, Math.max(groundH + 0.8, f.center.y));
        const fy = safeCy + f.yOffset + Math.sin(troutTime * f.speed * 2.2 + f.phase) * (f.vertAmp || 0.45);

        const dx = -Math.sin(currentAng) * f.radiusX * dir + Math.cos(troutTime * 0.18 + f.phase) * (f.wanderAmp || 6.0) * 0.18;
        const dz = Math.cos(currentAng) * f.radiusZ * dir - Math.sin(troutTime * 0.14 + f.phase * 1.3) * (f.wanderAmp || 6.0) * 0.14;
    ...
    if (this._koiMesh && this._koiData) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        const wanderX = Math.sin(koiTime * 0.15 + f.phase) * (f.wanderAmp || 8.0);
        const wanderZ = Math.cos(koiTime * 0.12 + f.phase * 1.2) * (f.wanderAmp || 8.0);
        const fx = f.center.x + Math.cos(currentAng) * f.radiusX + wanderX;
        const fz = f.center.z + Math.sin(currentAng) * f.radiusZ + wanderZ;
        if (refreshFishGround || f._cachedGH === undefined) f._cachedGH = terrainHeight(fx, fz);
        const groundH = f._cachedGH;
        ...
        const safeCy = Math.min(waterSurface - 0.8, Math.max(groundH + 0.8, f.center.y));
        const fy = safeCy + f.yOffset + Math.sin(koiTime * f.speed * 1.8 + f.phase) * (f.vertAmp || 0.45);

        const dx = -Math.sin(currentAng) * f.radiusX * dir + Math.cos(koiTime * 0.15 + f.phase) * (f.wanderAmp || 8.0) * 0.15;
        const dz = Math.cos(currentAng) * f.radiusZ * dir - Math.sin(koiTime * 0.12 + f.phase * 1.2) * (f.wanderAmp || 8.0) * 0.12;
        const heading = Math.atan2(dx, dz);
    ...
    if (this._reefFishMesh && this._reefFishData) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        const wanderX = Math.sin(fishTime * 0.20 + f.phase) * (f.wanderAmp || 8.0);
        const wanderZ = Math.cos(fishTime * 0.16 + f.phase * 1.4) * (f.wanderAmp || 8.0);
        const fx = f.center.x + Math.cos(currentAng) * f.radiusX + wanderX;
        const fz = f.center.z + Math.sin(currentAng) * f.radiusZ + wanderZ;
        if (refreshFishGround || f._cachedGH === undefined) f._cachedGH = terrainHeight(fx, fz);
        const groundH = f._cachedGH;
        ...
        const safeCy = Math.min(waterSurface - 0.7, Math.max(groundH + 0.7, f.center.y));
        const fy = safeCy + f.yOffset + Math.sin(fishTime * f.speed * 2.4 + f.phase) * (f.vertAmp || 0.50);

        const dx = -Math.sin(currentAng) * f.radiusX * dir + Math.cos(fishTime * 0.20 + f.phase) * (f.wanderAmp || 8.0) * 0.20;
        const dz = Math.cos(currentAng) * f.radiusZ * dir - Math.sin(fishTime * 0.16 + f.phase * 1.4) * (f.wanderAmp || 8.0) * 0.16;
        const heading = Math.atan2(dx, dz);
    ...
    if (this._dolphinMesh && this._dolphinData) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        const rawDy = d.center.y + d.yOffset + wave * 2.2;
        const dy = Math.max(d._cachedGH + 0.8, rawDy);
    ...
    if (this._sharkMesh && this._sharkData) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        const rawSy = s.center.y + s.yOffset + Math.sin(sharkTime * s.speed * 0.8 + s.phase) * 0.55;
        const sy = Math.max(s._cachedGH + 1.2, Math.min(-0.8, rawSy));
    ...
    if (this._seaTurtleMesh && this._seaTurtleData) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        const tx = tur.cx + Math.cos(currentAng) * tur.radiusX + wanderX;
        const tz = tur.cz + Math.sin(currentAng) * tur.radiusZ + wanderZ;
        const rawTy = tur.cy + Math.sin(turtleTime * tur.speed * 1.2 + tur.phase) * 0.65;

        const dx = -Math.sin(currentAng) * tur.radiusX * dir + Math.cos(turtleTime * 0.20 + tur.phase) * 6.0 * 0.20;
        const dz = Math.cos(currentAng) * tur.radiusZ * dir - Math.sin(turtleTime * 0.16 + tur.phase * 1.3) * 6.0 * 0.16;
        const heading = Math.atan2(dx, dz);

        if (refreshFishGround || tur._cachedGH === undefined) tur._cachedGH = terrainHeight(tx, tz);
        const ty = Math.max(tur._cachedGH + 0.6, Math.min(-0.5, rawTy));
    ...
    if (this._mantaRayMesh && this._mantaRayData) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        const mx = ray.cx + Math.cos(currentAng) * ray.radiusX + wanderX;
        const mz = ray.cz + Math.sin(currentAng) * ray.radiusZ + wanderZ;
        const rawMy = ray.cy + Math.sin(mantaTime * ray.speed * 0.8 + ray.phase) * 0.85;

        const dx = -Math.sin(currentAng) * ray.radiusX * dir + Math.cos(mantaTime * 0.16 + ray.phase) * 8.0 * 0.16;
        const dz = Math.cos(currentAng) * ray.radiusZ * dir - Math.sin(mantaTime * 0.13 + ray.phase * 1.2) * 8.0 * 0.13;
        const heading = Math.atan2(dx, dz);

        if (refreshFishGround || ray._cachedGH === undefined) ray._cachedGH = terrainHeight(mx, mz);
        const my = Math.max(ray._cachedGH + 1.0, Math.min(-1.2, rawMy));
    ...
    const isUnderTarn = (Math.abs(camX) < 62 && camZ <= -455 && camZ >= -575) && camY < 182.2;
    ...
    this._colSunlitAqua = this._colSunlitAqua || new THREE.Color(0x40c8f0);
    this._colDeepAbyssal = this._colDeepAbyssal || new THREE.Color(0x185878);
    this._colAbyssBg1 = this._colAbyssBg1 || new THREE.Color(0x185878);
    this._colAbyssBg2 = this._colAbyssBg2 || new THREE.Color(0x12425c);
    this._colTarnFog1 = this._colTarnFog1 || new THREE.Color(0x48c8f0);
    this._colTarnFog2 = this._colTarnFog2 || new THREE.Color(0x247898);
    this._colLakeFog1 = this._colLakeFog1 || new THREE.Color(0x38d8c8);
    this._colLakeFog2 = this._colLakeFog2 || new THREE.Color(0x1e7888);

    if (isUnderOcean) {
      // Exponential Beer-Lambert depth extinction from sunlit aquamarine to clear deep abyss
      const depth = Math.max(0.0, 0.35 - camY);
      const extinction = Math.exp(-depth * 0.025);

...
import { Surfaces, waterNormalTexture, textures, material, createBotanicalFoliageMaterial } from './materials.js';
...
export class World3D {
  ...
  _lights() {
    ...
    // Deep atmospheric sky top bounce (0x5078a0, 0.45) + rich ground bounce (0x384828, 0.30)
    ...
    sun.shadow.radius = 1.8; // Soft PCF penumbra filtering
  ...
  _updateLighting(phase, mood) {
    ...
    const m = mood || MOODS[this.mood] || MOODS.clear;

    // Dynamic Time-of-Day Spherical Solar Coordinates
    // Positioned at (-800, 950, 600) for standard daytime illumination
    const ELEV = { dawn: 8, day: 43.5, dusk: 6, night: 40 };
    const AZI = { dawn: 85, day: 307, dusk: 275, night: 45 };
    const elevation = ELEV[p.key] ?? 43.5;
    const azimuth = AZI[p.key] ?? 307;
    ...
    const SKY_PALETTE = {
      dawn: {
        ...
        sunCol: 0xffe0ba,   // Radiant morning champagne sunlight
        sunInt: 2.8,
      },
      day: {
        zenith: 0x093b7b,   // Rich celestial azure / cerulean sapphire zenith
        horizon: 0x8ac4f6,  // Radiant golden-tinted azure horizon
        ground: 0x384828,   // Deep rich meadow ground bounce
        sunCol: 0xfff4dc,   // Warm champagne gold sunlight
        sunInt: 3.6,
      },
      dusk: {
        zenith: 0x121c3e,   // Deep twilight sapphire-violet
        horizon: 0xea7236,  // Radiant molten coral & amber sunset horizon
        ground: 0x2e1e16,   // Warm terracotta shadow ground bounce
        sunCol: 0xff8a38,   // Molten gold-orange sunset
        sunInt: 2.8,
      },
      night: {
        zenith: 0x040c1e,   // Lapis-lazuli deep midnight space zenith
        horizon: 0x122444,  // Luminous celestial starlight horizon
        ground: 0x0a1412,   // Moonlit pasture ground bounce
        sunCol: 0xd4ecff,   // Pristine silver-cyan moonlight
        sunInt: 1.4,
      },
    }[p.key] || {
      zenith: 0x093b7b, horizon: 0x8ac4f6, ground: 0x384828, sunCol: 0xfff4dc, sunInt: 3.6
    };

    if (this.sky?.material?.uniforms) {
      const u = this.sky.material.uniforms;
      u.uSunPosition.value.copy(sunDir);
      u.uZenithColor.value.setHex(SKY_PALETTE.zenith);
      u.uHorizonColor.value.setHex(SKY_PALETTE.horizon);
      u.uGroundColor.value.setHex(SKY_PALETTE.ground);
      u.uSunColor.value.setHex(SKY_PALETTE.sunCol);
      u.uSunIntensity.value = SKY_PALETTE.sunInt;
    }
    ...
    if (!this.scene.background || !this.scene.background.isColor) {
      ...
      this.scene.background.set(SKY_PALETTE.zenith);
    }
    ...
    if (this._windMaterials) {
      for (let i = 0, len = this._windMaterials.length; i < len; i++) {
        ...
        if (m.userData?.botanicalShader?.uniforms?.uLightDir) {
          m.userData.botanicalShader.uniforms.uLightDir.value.copy(sunDir);
        }
        if (m.userData?.windShader?.uniforms?.uLightDir) {
          m.userData.windShader.uniforms.uLightDir.value.copy(sunDir);
        }
      }
    }
    ...
    if (this.scene.fog) {
      ...
      if (this.scene.fog.isFogExp2) {
        ...
        this.scene.fog.near = { clear: 1200, soft: 900, blessing: 600, crystal: 1100 }[this.mood] ?? 1200;
        this.scene.fog.far = { clear: 18000, soft: 14000, blessing: 9000, crystal: 16000 }[this.mood] ?? 18000;
      }
    }
    ...
    return { sunDir, LOOK, SKY_PALETTE };
  }
  ...
  _updateEnvironment() {
    ...
    if (this._envGroundPlane?.material?.color) {
      const phaseKey = this._forcedPhase?.key || (getDayPhase ? getDayPhase().key : 'day');
  ...
  _composer() {
    ...
    const bloom = new UnrealBloomPass(
      ...
      0.12,   // strength — subtle natural glow on specular peaks, golden inscriptions & eternal flame braziers
      0.38,   // radius — tight, physical highlight dispersion
      0.90,   // threshold — only peak specular glints and emissive brazier flames bloom, zero milky haze
    );
    this.bloomPass = bloom;
    composer.addPass(bloom);

    // 2. Hollywood ACES Filmic Tone Mapping & Accurate sRGB Output
    ...
    mat.onBeforeCompile = (shader) => {
      ...
      shader.fragmentShader = shader.fragmentShader
        ...
        .replace('#include <normal_fragment_begin>', `
          ...
          float slopeN = clamp(vWorldNormal.y, 0.0, 1.0);
          float cliffWeightN = 1.0 - smoothstep(0.35, 0.75, slopeN);

          // Triplanar rock normal projection (smooth cubic blend)
          vec3 blendingNorm = pow(abs(vWorldNormal), vec3(3.0));
          blendingNorm /= (blendingNorm.x + blendingNorm.y + blendingNorm.z + 0.0001);

          vec3 normX = texture2D(uCliffNorm, vWorldPosition.zy * 0.0035).xyz * 2.0 - 1.0;
          vec3 normY = texture2D(uCliffNorm, vWorldPosition.xz * 0.0035).xyz * 2.0 - 1.0;
          vec3 normZ = texture2D(uCliffNorm, vWorldPosition.xy * 0.0035).xyz * 2.0 - 1.0;
          vec3 cliffNorm = normalize(normX * blendingNorm.x + normY * blendingNorm.y + normZ * blendingNorm.z);

          // Snow normal mapping
          vec3 snowN = texture2D(uSnowMap, vWorldPosition.xz * 0.008).xyz * 2.0 - 1.0;
          vec3 snowWorldN = normalize(vec3(snowN.x * 0.75, 1.0, snowN.y * 0.75));

          float northAspect = clamp(-vWorldNormal.z * 0.55 + 0.45, 0.0, 1.0);
          float snowElevN = smoothstep(950.0 - northAspect * 140.0, 1420.0, vWorldPosition.y);
          ...
          vec3 blendedMountainNorm = normalize(
            mix(cliffNorm, snowWorldN, snowMaskN) * 0.65 + vWorldNormal * 0.35
          );
        ...
        .replace('#include <map_fragment>', `
          ...
          vec3 blendingCol = pow(abs(vWorldNormal), vec3(3.0));
          blendingCol /= (blendingCol.x + blendingCol.y + blendingCol.z + 0.0001);

          vec3 cliffX = texture2D(uCliffMap, vWorldPosition.zy * 0.0035).rgb * 0.65 +
                        texture2D(uCliffMap, vWorldPosition.zy * 0.015).rgb * 0.35;
          vec3 cliffY = texture2D(uCliffMap, vWorldPosition.xz * 0.0035).rgb * 0.65 +
                        texture2D(uCliffMap, vWorldPosition.xz * 0.015).rgb * 0.35;
          vec3 cliffZ = texture2D(uCliffMap, vWorldPosition.xy * 0.0035).rgb * 0.65 +
                        texture2D(uCliffMap, vWorldPosition.xy * 0.015).rgb * 0.35;
          ...
          vec3 screeCol = texture2D(uScreeMap, vWorldPosition.xz * 0.008).rgb * 1.10 + vec3(0.12, 0.11, 0.09);

          // Glacial firn snow albedo with subtle ice-blue shadows (#dce6f0 / #78a8cc)
          vec3 rawSnow = texture2D(uSnowMap, vWorldPosition.xz * 0.008).rgb * 0.70 +
                         texture2D(uSnowMap, vWorldPosition.xz * 0.024).rgb * 0.30;
          ...
          float nAspect = clamp(-vWorldNormal.z * 0.55 + 0.45, 0.0, 1.0);
          float snowElevation = smoothstep(950.0 - nAspect * 140.0, 1420.0, vWorldPosition.y);
          ...
          float snowHalfDot = max(0.0, dot(vWorldNormal, halfVec));
          ...
          float cloudDrift = sin(vWorldPosition.x * 0.0008 + vWorldPosition.z * 0.0006 + uTime * 0.05) *
                             cos(vWorldPosition.z * 0.0007 - vWorldPosition.x * 0.0004 - uTime * 0.035);
        ...
        .replace('#include <roughnessmap_fragment>', `
          ...
          float slopeR = clamp(vWorldNormal.y, 0.0, 1.0);
          float cliffR = 1.0 - smoothstep(0.35, 0.75, slopeR);
          float nAspectR = clamp(-vWorldNormal.z * 0.55 + 0.45, 0.0, 1.0);
          float snowElevR = smoothstep(950.0 - nAspectR * 140.0, 1420.0, vWorldPosition.y);
        ...
        .replace('#include <fog_fragment>', `
          ...
          float altitudeHaze = clamp(1.0 - (vWorldPosition.y - 200.0) / 1400.0, 0.40, 1.0);
          float aerialFactor = clamp((1.0 - exp(-distToCam * 0.00035)) * altitudeHaze, 0.0, 0.88);
          vec3 rayleighSkyHaze = vec3(0.5647, 0.7216, 0.8471); // 0x90b8d8 Rayleigh sky haze
  ...
  _terrain() {
    ...
    for (let i = 0; i < pos.count; i++) {
      ...
      // Exact central-difference analytical surface normal from continuous terrainHeight field
      const eps = 1.8;
      const hN = terrainHeight(x, z - eps);
      const hS = terrainHeight(x, z + eps);
      const hW = terrainHeight(x - eps, z);
      const hE = terrainHeight(x + eps, z);
      const nx = (hW - hE) / (2.0 * eps);
      const nz = (hN - hS) / (2.0 * eps);
      const ny = 1.0;
      const invLen = 1.0 / Math.hypot(nx, ny, nz);
      ...
      // Dual-radius vertex crevice ambient occlusion calculation (micro-fissures, couloirs, ravines & valleys)
      const epsFine = 2.0;
      const hN1 = terrainHeight(x, z - epsFine);
      const hS1 = terrainHeight(x, z + epsFine);
      const hW1 = terrainHeight(x - epsFine, z);
      const hE1 = terrainHeight(x + epsFine, z);
      const hNW1 = terrainHeight(x - epsFine * 0.707, z - epsFine * 0.707);
      const hNE1 = terrainHeight(x + epsFine * 0.707, z - epsFine * 0.707);
      const hSW1 = terrainHeight(x - epsFine * 0.707, z + epsFine * 0.707);
      const hSE1 = terrainHeight(x + epsFine * 0.707, z + epsFine * 0.707);
      const avgFine = (hN1 + hS1 + hW1 + hE1 + hNW1 + hNE1 + hSW1 + hSE1) * 0.125;
      const concavityFine = Math.max(0.0, avgFine - h);

      const epsMacro = 10.0;
      const hN2 = terrainHeight(x, z - epsMacro);
      const hS2 = terrainHeight(x, z + epsMacro);
      const hW2 = terrainHeight(x - epsMacro, z);
      const hE2 = terrainHeight(x + epsMacro, z);
      const hNW2 = terrainHeight(x - epsMacro * 0.707, z - epsMacro * 0.707);
      const hNE2 = terrainHeight(x + epsMacro * 0.707, z - epsMacro * 0.707);
      const hSW2 = terrainHeight(x - epsMacro * 0.707, z + epsMacro * 0.707);
      const hSE2 = terrainHeight(x + epsMacro * 0.707, z + epsMacro * 0.707);
      const avgMacro = (hN2 + hS2 + hW2 + hE2 + hNW2 + hNE2 + hSW2 + hSE2) * 0.125;
    ...
    mat.onBeforeCompile = (shader) => {
      ...
      shader.fragmentShader = shader.fragmentShader
        ...
        .replace('#include <normal_fragment_begin>', `
          ...
          vec3 gNorm1 = texture2D(uMeadowNorm, vWorldPosition.xz * 0.18).xyz * 2.0 - 1.0;
          vec3 gNorm2 = texture2D(uMeadowNorm, vWorldPosition.xz * 0.045).xyz * 2.0 - 1.0;
          vec3 gNorm3 = texture2D(uGroundDetailNorm, vWorldPosition.xz * 0.06).xyz * 2.0 - 1.0;
          ...
          vec3 sNorm1 = texture2D(uCliffNorm, vWorldPosition.xz * 0.09).xyz * 2.0 - 1.0;
          vec3 sNorm2 = texture2D(uCliffNorm, vWorldPosition.xz * 0.022).xyz * 2.0 - 1.0;
          ...
          vec3 blendingNorm = pow(abs(vWorldNormal), vec3(6.0));
          blendingNorm /= (blendingNorm.x + blendingNorm.y + blendingNorm.z + 0.0001);

          vec3 normX1 = texture2D(uCliffNorm, vWorldPosition.zy * 0.040).xyz * 2.0 - 1.0;
          vec3 normY1 = texture2D(uCliffNorm, vWorldPosition.xz * 0.040).xyz * 2.0 - 1.0;
          vec3 normZ1 = texture2D(uCliffNorm, vWorldPosition.xy * 0.040).xyz * 2.0 - 1.0;

          vec3 normX2 = texture2D(uCliffNorm, vWorldPosition.zy * 0.010).xyz * 2.0 - 1.0;
          vec3 normY2 = texture2D(uCliffNorm, vWorldPosition.xz * 0.010).xyz * 2.0 - 1.0;
          vec3 normZ2 = texture2D(uCliffNorm, vWorldPosition.xy * 0.010).xyz * 2.0 - 1.0;

          vec3 normX3 = texture2D(uCliffNorm, vWorldPosition.zy * 0.12).xyz * 2.0 - 1.0;
          vec3 normY3 = texture2D(uCliffNorm, vWorldPosition.xz * 0.12).xyz * 2.0 - 1.0;
          vec3 normZ3 = texture2D(uCliffNorm, vWorldPosition.xy * 0.12).xyz * 2.0 - 1.0;
          ...
          vec3 cliffWorldX = vec3(0.0, normX.y, normX.x * sign(vWorldNormal.x));
          vec3 cliffWorldY = vec3(normY.x, 0.0, normY.y * sign(vWorldNormal.y));
          vec3 cliffWorldZ = vec3(normZ.x * sign(vWorldNormal.z), normZ.y, 0.0);
          vec3 cliffWorldNorm = normalize(vWorldNormal + (cliffWorldX * blendingNorm.x + cliffWorldY * blendingNorm.y + cliffWorldZ * blendingNorm.z) * 2.2);

          // 4. Snowy Mountain Ridge Caps normal mapping (extreme heights > 280m only)
          vec3 snowNorm1 = texture2D(uMeadowNorm, vWorldPosition.xz * 0.08).xyz * 2.0 - 1.0;
          vec3 snowNorm2 = texture2D(uMeadowNorm, vWorldPosition.xz * 0.02).xyz * 2.0 - 1.0;
          ...
          float snowElevationN = smoothstep(280.0, 360.0, vWorldPosition.y + sin(vWorldPosition.x * 0.015 + vWorldPosition.z * 0.015) * 6.0);
        ...
        .replace('#include <map_fragment>', `
          ...
          // 1. Deep Rich Fescue Grass Albedo (#2e5c1e base, #4a882a sunlit crest, #183410 soil shadow + groundDetail + meadowLush)
          vec3 fescueSoil   = vec3(0.0941, 0.2039, 0.0627); // #183410 soil shadow
          vec3 fescueBase   = vec3(0.1804, 0.3608, 0.1176); // #2e5c1e rich fescue base
          vec3 fescueSunlit = vec3(0.2902, 0.5333, 0.1647); // #4a882a sunlit crest

          vec3 grassFine   = texture2D(uMeadowMap, vWorldPosition.xz * 0.18).rgb;
          vec3 grassMid    = texture2D(uMeadowMap, vWorldPosition.xz * 0.045).rgb;
          vec3 grassDrift  = texture2D(uMeadowMap, vWorldPosition.xz * 0.010).rgb;
          vec3 groundMod   = texture2D(uGroundDetail, vWorldPosition.xz * 0.06).rgb;

          // Composite multi-octave grass blade albedo
          vec3 meadowTexComposite = grassFine * (grassMid * 1.20 + 0.30) * (grassDrift * 0.70 + 0.65) * (groundMod * 0.85 + 0.35);

          // Blend fescue palette by blade height and lighting
          float bladeHeight = (grassFine.g + grassMid.g * 0.5) / 1.5;
          vec3 fescueColor = mix(fescueSoil, fescueBase, smoothstep(0.12, 0.48, bladeHeight));
          fescueColor = mix(fescueColor, fescueSunlit, smoothstep(0.48, 0.86, bladeHeight));

          vec3 meadowAlbedo = fescueColor * meadowTexComposite * 2.2;

          // Subtle tonal drifts: golden clover accents and cool shaded emerald
          float macroColorDrift = sin(vWorldPosition.x * 0.012 + vWorldPosition.z * 0.008) * cos(vWorldPosition.z * 0.010);
          ...
          float cloverPatch = pow(texture2D(uMeadowMap, vWorldPosition.xz * 0.045).g, 2.4);
          ...
          vec3 viewDir = normalize(cameraPosition - vWorldPosition);
          vec3 halfVec = normalize(sunDir + viewDir);
          float bladeNdotH = max(0.0, dot(vWorldNormal, halfVec));
          ...
          vec3 screeFine = texture2D(uScreeMap, vWorldPosition.xz * 0.09).rgb;
          vec3 screeMacro = texture2D(uScreeMap, vWorldPosition.xz * 0.022).rgb;
          ...
          vec3 blendingCol = pow(abs(vWorldNormal), vec3(6.0));
          blendingCol /= (blendingCol.x + blendingCol.y + blendingCol.z + 0.0001);

          vec3 cliffX1 = texture2D(uCliffMap, vWorldPosition.zy * 0.040).rgb;
          vec3 cliffY1 = texture2D(uCliffMap, vWorldPosition.xz * 0.040).rgb;
          vec3 cliffZ1 = texture2D(uCliffMap, vWorldPosition.xy * 0.040).rgb;
          vec3 cliffDetail1 = cliffX1 * blendingCol.x + cliffY1 * blendingCol.y + cliffZ1 * blendingCol.z;

          vec3 cliffX2 = texture2D(uCliffMap, vWorldPosition.zy * 0.010).rgb;
          vec3 cliffY2 = texture2D(uCliffMap, vWorldPosition.xz * 0.010).rgb;
          vec3 cliffZ2 = texture2D(uCliffMap, vWorldPosition.xy * 0.010).rgb;
          vec3 cliffDetail2 = cliffX2 * blendingCol.x + cliffY2 * blendingCol.y + cliffZ2 * blendingCol.z;

          vec3 cliffX3 = texture2D(uCliffMap, vWorldPosition.zy * 0.12).rgb;
          vec3 cliffY3 = texture2D(uCliffMap, vWorldPosition.xz * 0.12).rgb;
          vec3 cliffZ3 = texture2D(uCliffMap, vWorldPosition.xy * 0.12).rgb;
          ...
          vec3 splatAlbedo = meadowAlbedo * meadowFactor + screeAlbedo * screeFactor + triplanarCliff * cliffFactor;

          // Harmonize with macro terrain vertex color smoothly (rich color depth)
          vec3 groundBase = splatAlbedo * (vTerrainColor * 1.25 + 0.10);

          // 4. Delicate Snow-Capped Mountain Peaks with Ice-Blue Subsurface Scattering on Extreme Heights > 280m
          float snowElevation = smoothstep(280.0, 360.0, vWorldPosition.y + sin(vWorldPosition.x * 0.015 + vWorldPosition.z * 0.015) * 6.0);
          float snowSlope = smoothstep(0.55, 0.85, slope);
          float snowFactor = clamp(snowElevation * snowSlope, 0.0, 1.0);

          vec3 snowFine = texture2D(uSnowMap, vWorldPosition.xz * 0.08).rgb;
          vec3 snowMacro = texture2D(uSnowMap, vWorldPosition.xz * 0.02).rgb;
          vec3 snowAlbedo = snowFine * (snowMacro * 0.85 + 0.35);

          // Ice-blue Subsurface Scattering forward-scattering glow
          float snowSunDot = max(0.0, dot(vWorldNormal, sunDir));
          ...
          vec2 orthoUv = (vWorldPosition.xz + vec2(1000.0, 1000.0)) / 2000.0;
          ...
          float isWaterfallChasm = (1.0 - smoothstep(24.0, 75.0, abs(vWorldPosition.x))) * smoothstep(-580.0, -290.0, vWorldPosition.z);
          if (isWaterfallChasm > 0.001) {
            ...
            float gorgeSpec = pow(max(0.0, dot(vWorldNormal, halfVec)), 24.0) * 0.35 * (1.0 - shelfSpray * 0.5);
            gorgeRock += vec3(0.70, 0.85, 0.95) * gorgeSpec;
            groundBase = mix(groundBase, gorgeRock, isWaterfallChasm * 0.88);
          }

          // Shoreline wet sand transition near water level (h < 13.5)
          float waterDist = max(0.0, vWorldPosition.y - 12.5);
          float shorelineWet = (1.0 - smoothstep(0.0, 1.4, waterDist)) * (1.0 - cliffFactor);
          ...
          float cloudDrift1 = sin(vWorldPosition.x * 0.0016 + vWorldPosition.z * 0.0012 + uTime * 0.075) *
                              cos(vWorldPosition.z * 0.0014 - vWorldPosition.x * 0.0006 - uTime * 0.045);
          float cloudDrift2 = sin(vWorldPosition.x * 0.0034 - vWorldPosition.z * 0.0028 - uTime * 0.09) * 0.35;
        ...
        .replace('#include <roughnessmap_fragment>', `
          #include <roughnessmap_fragment>
          float slopeRough = clamp(vWorldNormal.y, 0.0, 1.0);
          ...
          float snowElevR = smoothstep(280.0, 360.0, vWorldPosition.y + sin(vWorldPosition.x * 0.015 + vWorldPosition.z * 0.015) * 6.0);
          float snowSlopeR = smoothstep(0.55, 0.85, slopeRough);
          float snowMaskR = clamp(snowElevR * snowSlopeR, 0.0, 1.0);
          terrainRough = mix(terrainRough, 0.65, snowMaskR);

          float isWFallChasm = (1.0 - smoothstep(24.0, 75.0, abs(vWorldPosition.x))) * smoothstep(-580.0, -290.0, vWorldPosition.z);
          float wDist = max(0.0, vWorldPosition.y - 12.5);
          float sWet = (1.0 - smoothstep(0.0, 1.4, wDist)) * (1.0 - (cFactorRough / tFactorRough));
    ...
    mat.onBeforeCompile = (shader) => {
      ...
      shader.vertexShader = shader.vertexShader.replace(
        ...
        `#include <begin_vertex>
         ...
         ` : ''}
         ${type === 'lake' ? `
         float distCenter = length(transformed.xz - vec2(430.0, -260.0));
  ...
  _createRiverMaterial(normals) {
    ...
    const riverFrag = `
      ...
      void main() {
        ...
        vec3 surfaceNormal = normalize(vWorldNormal + waveNormal * 0.38);
  ...
  _createFountainBasinMaterial(normals) {
    ...
    const frag = `
      ...
      void main() {
        ...
        vec3 surfaceNormal = normalize(vWorldNormal + n * 0.35);
  ...
  _water() {
    ...
    const normals = (normTex.normalMap || normTex.normal).clone();
    ...
    const troutSchools = [
      { cx: 0, cy: 15.0, cz: -360, radiusX: 18, radiusZ: 22, count: 45, type: 'glacial_trout' }, // Plunge pool (waterLevel 18.0)
      { cx: 0, cy: 176.0, cz: -505, radiusX: 22, radiusZ: 28, count: 55, type: 'glacial_trout' }, // Glacial Tarn (waterLevel 182.0)
    ...
    // =========================================================================
    // 4. Kaya Island & Pelagic Lagoon: 168+ Tropical Reef Fish (_buildReefFishMesh)
    // =========================================================================
    const reefFishGeo = this._buildReefFishMesh();
    const reefSchools = [
      // Kaya Island Coral Reef Lagoon (Directly along dive path x = 35 -> 0 -> -30, z = 2190 -> 2270 -> 2240)
      { cx: 14,  cy: -5.2, cz: 2235, radiusX: 22, radiusZ: 26, count: 72, type: 'reef_clownfish' }, // Coral Clownfish & Anthias (plunge entry WP 80-81)
      { cx: -10, cy: -5.4, cz: 2265, radiusX: 24, radiusZ: 28, count: 80, type: 'reef_tangs' },      // Neon Azure Tangs, Yellow Tangs & Moorish Idols (WP 81)
      { cx: -26, cy: -5.3, cz: 2240, radiusX: 20, radiusZ: 24, count: 64, type: 'reef_beauties' },   // Coral Beauties & Mint Wrasses (return WP 82)
      { cx: 0,   cy: -6.8, cz: 2310, radiusX: 42, radiusZ: 48, count: 48, type: 'pelagic_jacks' },   // Pelagic Sapphire Jacks in deep lagoon shelf

      // Coastal Ocean Reef (z = 980 to 1320)
      { cx: 45,  cy: -4.2, cz: 1160, radiusX: 45, radiusZ: 50, count: 48, type: 'reef_clownfish' }, // Clownfish & Anthias
      { cx: -40, cy: -4.0, cz: 1100, radiusX: 42, radiusZ: 48, count: 40, type: 'reef_tangs' },      // Blue Tangs & Damselfish
    ];
    ...
    // =========================================================================
    // 7. Submerged Cold Water & Deep Ocean Aeration Bubble System (GPU Points)
    // =========================================================================
  ...
  _oceanWaterfall() {
    ...
    // 3. Volumetric Spray & Rising Mist Particles with Physical Solar Rainbow Glints
    const mistCount = 220;
    const mistGeo = new THREE.BufferGeometry();
    const mistPos = new Float32Array(mistCount * 3);
    const mistPhase = new Float32Array(mistCount);
    const mistVel = new Float32Array(mistCount * 3);
    const mistScale = new Float32Array(mistCount);
    const mistRange = new Float32Array(mistCount);
    for (let i = 0; i < mistCount; i++) {
      mistPos[i * 3]     = cliffX + 2.5 + (Math.random() - 0.5) * 28;
      mistPos[i * 3 + 1] = bottomY + Math.random() * (fallHeight * 0.85);
      mistPos[i * 3 + 2] = cliffZ + 36 + (Math.random() - 0.5) * 28;
      mistPhase[i] = Math.random() * Math.PI * 2;
      mistVel[i * 3]     = (Math.random() - 0.5) * 6.0;
      mistVel[i * 3 + 1] = 3.5 + Math.random() * 5.0;
      mistVel[i * 3 + 2] = (Math.random() - 0.5) * 6.0;
      mistScale[i]       = 0.6 + Math.random() * 0.8;
      mistRange[i]       = 18.0 + Math.random() * 12.0;
    }
    mistGeo.setAttribute('position', new THREE.BufferAttribute(mistPos, 3));
    mistGeo.setAttribute('aPhase', new THREE.BufferAttribute(mistPhase, 1));
    mistGeo.setAttribute('aVel', new THREE.BufferAttribute(mistVel, 3));
    mistGeo.setAttribute('aScale', new THREE.BufferAttribute(mistScale, 1));
    mistGeo.setAttribute('aRange', new THREE.BufferAttribute(mistRange, 1));
    mistGeo.computeBoundingSphere();
    const mistPoints = new THREE.Points(mistGeo, this._mistShader);
  ...
  _universalCathedral() {
    ...
    const upperPodium = new THREE.Mesh(new THREE.BoxGeometry(60, 1.4, 110), stone);
    upperPodium.position.set(0, 2.1, -4);
    ...
    const northTerraceUpper = new THREE.Mesh(new THREE.BoxGeometry(18, 1.4, 24), stone);
    northTerraceUpper.position.set(-39, 2.1, -4);
    ...
    [-34.2, 34.2].forEach(tx => {
      ...
      tGableGroup.position.set(tx, 38.0, tx > 0 ? 8.2 : -8.2);
    ...
    const apseWallGeo = new THREE.CylinderGeometry(11.0, 11.0, 36, 16, 1, false, Math.PI / 2, Math.PI);
    ...
    buttressZPlacements.forEach(bz => {
      [-23.5, 23.5].forEach(bx => {
        ...
        // Upper flyer bracing high clerestory wall
        const upperFlyer = new THREE.Mesh(new THREE.TorusGeometry(8.5, 0.45, 10, 24, Math.PI * 0.42), stone);
        upperFlyer.position.set(-sign * 6.2, 28.5, 0);
        upperFlyer.rotation.z = sign > 0 ? Math.PI * 0.58 : Math.PI * 0.0;
        upperFlyer.castShadow = true;
        buttressGroup.add(upperFlyer);

        // Lower flyer bracing vault thrust
        const lowerFlyer = new THREE.Mesh(new THREE.TorusGeometry(7.2, 0.40, 10, 24, Math.PI * 0.38), stone);
        lowerFlyer.position.set(-sign * 5.4, 21.0, 0);
        lowerFlyer.rotation.z = sign > 0 ? Math.PI * 0.60 : Math.PI * 0.02;
    ...
    // Soaring Vaulted Ceiling (Michelangelo Sistine Fresco Celestial Vaults)
    ...
    vaultCeiling.position.set(0, 36.0, -4);
    g.add(vaultCeiling);
    ...
    const rayMat = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        void main() {
          ...
          vec4 wPos = modelMatrix * vec4(position, 1.0);
          vWorldNormal = normalize((modelMatrix * vec4(normal, 0.0)).xyz);
          vToEye = normalize(cameraPosition - wPos.xyz);
          gl_Position = projectionMatrix * viewMatrix * wPos;
        }
      `,
      fragmentShader: `
        ...
        void main() {
          float viewAngle = max(0.0, dot(vWorldNormal, vToEye));
  ...
  _gate() {
    ...
    const createFloralUrn = (scale = 1.0) => {
      ...
      // 2. Sculpted Carrara marble pedestal & molded base
      const urnPed = new THREE.Mesh(new THREE.BoxGeometry(3.6 * scale, 1.0 * scale, 3.6 * scale), marble);
      urnPed.position.y = 0.5 * scale;
      urnGroup.add(urnPed);

      const urnFoot = new THREE.Mesh(new THREE.CylinderGeometry(1.6 * scale, 1.0 * scale, 1.2 * scale, 16), gold);
      urnFoot.position.y = 1.6 * scale;
      urnGroup.add(urnFoot);

      const urnBowl = new THREE.Mesh(new THREE.CylinderGeometry(3.2 * scale, 1.6 * scale, 2.6 * scale, 24), marble);
      urnBowl.position.y = 3.4 * scale;
      urnGroup.add(urnBowl);

      const urnRim = new THREE.Mesh(new THREE.TorusGeometry(3.3 * scale, 0.35 * scale, 12, 32), gold);
      urnRim.rotation.x = Math.PI / 2;
      urnRim.position.y = 4.7 * scale;
      urnGroup.add(urnRim);

      for (let s of [-1, 1]) {
        const handle = new THREE.Mesh(new THREE.TorusGeometry(1.4 * scale, 0.28 * scale, 8, 16, Math.PI * 1.2), gold);
        handle.rotation.z = s * 0.45;
        handle.position.set(s * 3.2 * scale, 3.4 * scale, 0);
        urnGroup.add(handle);
      }

      const soilBed = new THREE.Mesh(new THREE.CylinderGeometry(2.9 * scale, 2.7 * scale, 0.6 * scale, 16), soilMat);
      soilBed.position.y = 4.5 * scale;
      urnGroup.add(soilBed);

      // 3. Overflowing Cascading Ivy Foliage Draping over Urn Rim (14 curved draping leaf cards)
      for (let f = 0; f < 14; f++) {
        const fAng = (f / 14) * Math.PI * 2;
        ...
        const fMesh = new THREE.Mesh(droopCard, floralFoliageMat);
        fMesh.castShadow = true;
        urnGroup.add(fMesh);
      }

      // 4. Blooming Rose and Lily Flowers (20 sculpted flower heads in lush dome mound)
      for (let fl = 0; fl < 20; fl++) {
        const flPhi = Math.acos(1 - 2 * ((fl + 0.5) / 20));
        ...
        const rMat = roseMats[fl % roseMats.length];
        const flHead = new THREE.Group();
        for (let p = 0; p < 5; p++) {
          const pAng = (p / 5) * Math.PI * 2;
          const petal = new THREE.Mesh(new THREE.SphereGeometry(0.24 * scale, 8, 6), rMat);
          petal.scale.set(1.0, 0.35, 1.2);
          petal.position.set(Math.cos(pAng) * 0.18 * scale, 0, Math.sin(pAng) * 0.18 * scale);
          petal.rotation.y = pAng;
          petal.rotation.x = 0.35;
          flHead.add(petal);
        }
        const flCenter = new THREE.Mesh(new THREE.SphereGeometry(0.16 * scale, 8, 8), roseMats[2]);
        flHead.add(flCenter);
        flHead.position.set(fx, fy, fz);
        flHead.rotation.set((fl % 5) * 0.15, flTheta, (fl % 3) * 0.15);
        urnGroup.add(flHead);
      }
    ...
    const terraceBase = new THREE.Mesh(new THREE.BoxGeometry(HALF * 2 + 220, 6.0, 72), stoneLight);
    terraceBase.position.set(0, -2.8, 0);
    terraceBase.receiveShadow = true;
    g.add(terraceBase);

    // Carved Carrara marble threshold apron spanning central entrance
    const thresholdApron = new THREE.Mesh(new THREE.BoxGeometry(HALF * 2 + 36, 1.6, 42), marble);
    thresholdApron.position.set(0, 0.45, 0);
    ...
    left.rotation.y = -1.15; // Swung open welcoming visitors into the valley promenade
    g.add(left);

    const right = gateLeaf(false);
    right.position.set(HALF - 0.5, 1.0, 0);
    right.rotation.y = Math.PI + 1.15; // Swung open welcoming visitors into the valley promenade
    g.add(right);

    this._gateLeft = left;
    this._gateRight = right;
    this.gateOpenAmount = 0.85;
    this.gateTargetOpen = 0.85;
  ...
  _plaza() {
    ...
    for (let side of [-1, 1]) {
      for (let b = 0; b < 4; b++) {
        ...
        bollardGroup.position.set(bx, 1.6, bz);

        const bBase = new THREE.Mesh(new THREE.BoxGeometry(1.6, 1.0, 1.6), graniteDark);
        bBase.position.y = 0.5;
        bollardGroup.add(bBase);

        const bShaft = new THREE.Mesh(new THREE.CylinderGeometry(0.48, 0.60, 2.2, 12), marble);
        bShaft.position.y = 2.1;
        bollardGroup.add(bShaft);

        const bCapRing = new THREE.Mesh(new THREE.TorusGeometry(0.65, 0.14, 8, 16), gold);
        bCapRing.rotation.x = Math.PI / 2;
        bCapRing.position.y = 3.2;
        bollardGroup.add(bCapRing);

        const bLantern = new THREE.Mesh(new THREE.DodecahedronGeometry(0.85, 0), lanternMat);
        bLantern.position.y = 4.0;
        bollardGroup.add(bLantern);

        const bCore = new THREE.Mesh(new THREE.SphereGeometry(0.42, 8, 8), lanternCoreMat);
        bCore.position.y = 4.0;
        bollardGroup.add(bCore);

        if (this._glowTex) {
          const bHalo = new THREE.Sprite(haloMat);
          bHalo.position.y = 4.0;
          bHalo.scale.set(5.5, 5.5, 1.0);
          bollardGroup.add(bHalo);
        }

        const bCage = new THREE.Mesh(new THREE.BoxGeometry(1.1, 1.3, 1.1), darkBronze);
        bCage.position.y = 4.0;
        bollardGroup.add(bCage);

        const bCap = new THREE.Mesh(new THREE.ConeGeometry(0.95, 0.9, 4), gold);
        bCap.rotation.y = Math.PI / 4;
        bCap.position.y = 5.0;
        bollardGroup.add(bCap);
  ...
  _rainbowBridge() {
    ...
    // 2. Arched deck: 28 smooth curved Carrara marble & flagstone segments with Celestial Starlight Paving
    const SEG = 28;
    for (let i = 0; i < SEG; i++) {
      const t = i / (SEG - 1);
      const z = bz - 60 + t * 120;
      ...
      const seg = new THREE.Mesh(segGeo, stone);
      seg.position.set(bx, y, z);
      seg.rotation.x = -Math.cos(t * Math.PI) * 0.15;
      seg.castShadow = seg.receiveShadow = true;
      g.add(seg);

      // Celestial Starlight Paving: Inlaid 24K gold bronze dividers and luminous central carriageway strip
      const starlightStrip = new THREE.Mesh(
        new THREE.BoxGeometry(1.2, 0.08, 120 / SEG + 0.4),
      ...
      starlightStrip.position.set(bx, y + 0.74, z);
      starlightStrip.rotation.x = -Math.cos(t * Math.PI) * 0.15;
      g.add(starlightStrip);

      // Inlaid gold star markers on alternate segments
      if (i % 2 === 0) {
        for (const starSide of [-6.0, 6.0]) {
          const starMarker = new THREE.Mesh(new THREE.OctahedronGeometry(0.35, 0), goldTrim);
          starMarker.position.set(bx + starSide, y + 0.75, z);
          starMarker.rotation.x = -Math.cos(t * Math.PI) * 0.15;
          g.add(starMarker);
        }
      }

      // Pierced Openwork Crystal Balustrade along Bridge Deck
      for (const s of [-1, 1]) {
        // Honed Carrara marble balustrade plinth
        const bPlinth = new THREE.Mesh(new THREE.BoxGeometry(1.6, 0.8, 110 / SEG + 0.9), marble);
        bPlinth.position.set(bx + s * 15, y + 1.1, z);
        bPlinth.rotation.x = -Math.cos(t * Math.PI) * 0.15;
        bPlinth.receiveShadow = true;
        g.add(bPlinth);

        // Pierced Openwork Crystal Balusters (Faceted optical crystal)
        for (let b = -1; b <= 1; b++) {
          const balZ = z + b * (120 / (SEG * 3));
          const balY = this._deckY(balZ) || y;
          const crystalBaluster = new THREE.Mesh(new THREE.CylinderGeometry(0.26, 0.36, 1.8, 8), crystalBalusterMat);
          crystalBaluster.position.set(bx + s * 15, balY + 2.3, balZ);
          crystalBaluster.castShadow = true;
          g.add(crystalBaluster);
        }

        // Classical Carrara marble handrail
        const rail = new THREE.Mesh(new THREE.BoxGeometry(1.6, 0.6, 120 / SEG + 0.9), marble);
        rail.position.set(bx + s * 15, y + 3.4, z);
        rail.rotation.x = -Math.cos(t * Math.PI) * 0.15;
        g.add(rail);

        // 24K Golden bronze filigree capping rail
        const goldCap = new THREE.Mesh(new THREE.BoxGeometry(1.4, 0.25, 120 / SEG + 0.8), goldTrim);
        goldCap.position.set(bx + s * 15, y + 3.8, z);
        goldCap.rotation.x = -Math.cos(t * Math.PI) * 0.15;
        g.add(goldCap);
      }
    }
  ...
  applyAmbience() {
    ...
    if (this.waterObjects) {
      for (const w of this.waterObjects) {
        if (w.material?.uniforms) {
          ...
          if (wu.distortionScale) wu.distortionScale.value = M.rainbow > 0.7 ? 4.2 : 3.8;
    ...
    // --- Bloom: soft and subtle, zero milky haze ---
    if (this.bloomPass) {
      this.bloomPass.strength = 0.10 + P.night * 0.28 + M.rainbow * 0.12;
      this.bloomPass.radius = 0.50 + P.night * 0.20;
      this.bloomPass.threshold = phase.key === 'night' ? 0.74 : 0.94;
    }

    // The rainbow: delicate atmospheric dispersion arc
    this._rainbowBase = (0.08 + 0.10 * M.rainbow) * (1 + P.night * 0.25);
    this._pawBase = 0.28;
  }
  ...
  _vegetation() {
    ...
    const place = (x, z) => {
      ...
      if (distToRiver(x, z) < 12) return null; // keep river channels clear of tree trunks
    ...
    for (let i = 0; i < 1800; i++) {
      ...
      const tiltX = -Math.atan2(hZ, 4.0) * 0.35;
      const tiltZ = Math.atan2(hX, 4.0) * 0.35;

      const altScale = Math.max(0.70, 1.0 - Math.max(0, mh - 110) / 220);
      const s = (1.35 + rngMF() * 1.65) * altScale;

      tmp.position.set(mx, mh, mz);
    ...
    const pineTex = textures('pineNeedles');
    const pineMat = createBotanicalFoliageMaterial(0x284f24, pineTex.map, {
      isTree: true,
      normalMap: pineTex.normalMap,
      normalScale: 1.2,
      roughness: 0.76,
  ...
  _meadowCarpet() {
    ...
    // 1. Concentrated alpine pasture grass tufts (3,000 instances across active sanctuary floor)
    ...
    // 2. Clustered Wildflowers & Ground Clutter (1,400 instances along pathways and viewpoints)
    ...
    // 3-blade interlocking cross-fan grass tuft geometry for rich volume
    const grassGeo = (() => {
      const parts = [];
      for (let b = 0; b < 3; b++) {
        const a = (b / 3) * Math.PI;
        const q = new THREE.PlaneGeometry(0.95, 1.05);
        q.translate(0, 0.525, 0);
        q.rotateY(a);
        parts.push(q);
      }
      return mergeGeometries(parts, false) || parts[0];
    ...
    const grassMat = Surfaces.grassTuft(0x56963e);
    grassMat.roughness = 0.92;
    grassMat.alphaTest = 0.5;
  ...
  _initAmbienceControls() {
    ...
    if (!pill) return;
    pill.querySelectorAll('.sap-btn[data-phase]').forEach(b => b.classList.toggle('is-active', b.dataset.phase === (this._forcedPhase?.key || 'day')));
  ...
  _initTourSpline() {
    ...
    const tourPoints = [
      ...
      new THREE.Vector3(0, 42.5, 700),    // 5: Gliding down imperial avenue
      new THREE.Vector3(0, 42.0, 630),    // 6: Approaching north avenue garden median
    ...
    this._tourSpline = new THREE.CatmullRomCurve3(tourPoints, true, 'centripetal', 0.4);
    this._tourTime = 0;
    this._tourSpeed = 1.0;
    this._tourPaused = false;
    this._currentRoll = 0.0;
  ...
  _calculateTourLookTarget(t, pos, tangent, outTarget) {
    ...
    if (normT < S) {
      if (normT < S * 0.5) {
        target.set(0.0, 38.0, 880.0);
      } else {
        target.copy(pos).addScaledVector(tan, 45.0);
      }
      return target;
    }
    ...
    if (normT < 2 * S) {
      if (normT < S + S * 0.6) {
        target.set(0.0, 22.5, pos.z - 48.0);
      } else {
        const frac = smooth((normT - (S + S * 0.6)) / (S * 0.4));
        const bz = pos.z - 48.0;
        target.set(0.0, 22.5 + (20.0 - 22.5) * frac, bz + (20.0 - bz) * frac);
      }
      return target;
    }
    ...
    if (normT < 8 * S) {
      if (normT < 7 * S + S * 0.4) {
        ...
        target.set(-300.0 + (400.0 - -300.0) * frac, 85.0 + (12.5 - 85.0) * frac, -210.0 + (-270.0 - -210.0) * frac);
    ...
    if (normT < 1.0) {
      if (normT < 10 * S + S * 0.1) {
        ...
        target.set(0.0, 75.0 + (42.0 - 75.0) * frac, 420.0 + (920.0 - 420.0) * frac);
      }
    }
  ...
  nextTourStage() {
    ...
    const currentT = ((this._tourTime % 1.0) + 1.0) % 1.0;
    let nextIdx = 0;
    for (let i = 0; i < this._tourStages.length; i++) {
      if (currentT < this._tourStages[i].tEnd - 0.01) {
        nextIdx = (i + 1) % this._tourStages.length;
        break;
      }
    }
  ...
  prevTourStage() {
    if (!this._tourStages || this._tourStages.length === 0) return;
    const currentT = ((this._tourTime % 1.0) + 1.0) % 1.0;
    let prevIdx = this._tourStages.length - 1;
    for (let i = 0; i < this._tourStages.length; i++) {
      const s = this._tourStages[i];
      if (currentT >= s.tStart && (currentT < s.tEnd || i === this._tourStages.length - 1)) {
        if (currentT - s.tStart > 0.02) {
          prevIdx = i;
        } else {
          prevIdx = (i - 1 + this._tourStages.length) % this._tourStages.length;
        }
        break;
      }
    }
  ...
  setTourStage(index) {
    ...
    // Immediate camera positioning at stage waypoint
  ...
  _updateWalk(dt) {
    ...
    if (this.tourMode) {
      ...
      let poiEase = 1.0;
      if (this._tourStages && this._tourStages.length > 0) {
        for (let i = 0; i < this._tourStages.length; i++) {
          const st = this._tourStages[i];
          if (currentNormT >= st.tStart && currentNormT < st.tEnd) {
            const frac = (currentNormT - st.tStart) / (st.tEnd - st.tStart);
            const scale = st.speedScale || 0.82;
            poiEase = scale + (1.0 - scale) * 0.5 * (1.0 + Math.cos(frac * Math.PI * 2));
            break;
          }
        }
      }
      ...
      this._tourTime += safeDt * 0.0112 * this._tourSpeed * poiEase * speedMult;
  ...
  _animate() {
    ...
    if (view3d && view3d.style.display === 'none' && !view3d.classList.contains('is-entering')) return;
    if (!this.renderer || !this.scene || !this.camera) return;
    ...
    if (this._gateLeft && this._gateRight) {
      ...
      if (this.tourMode) {
        shouldOpen = this.gateTargetOpen === 1.0 || (camZ < 940) || this._forceGateOpen;
      ...
      // Smooth physical hinge rotation with exponential damping (0 = closed, 1 = wide open)
      const hingeSpeed = shouldOpen ? 1.8 : 1.5;
    ...
    // Silky-smooth 60 FPS native hardware render path
    // WebGLRenderer features native hardware MSAA, AgX/ACESFilmic Tone Mapping, and SRGB color space directly on GPU
  ...
  _mountainWaterfall() {
    ...
    const waterfallShader = {
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          // Specular highlights
          vec3 viewDir = normalize(cameraPosition - vWorldPos);
          vec3 halfVector = normalize(vec3(0.4, 0.8, 0.5) + viewDir);
    ...
    const veilPoints = fallPoints.map(p => new V3(p.x, p.y - 0.4, p.z + 1.4));
    ...
    const mistPhase = new Float32Array(mistCount);
    for (let i = 0; i < mistCount; i++) {
      mistPos[i * 3]     = (Math.random() - 0.5) * 52;
      mistPos[i * 3 + 1] = Math.random() * 42;
      mistPos[i * 3 + 2] = -360 + (Math.random() - 0.5) * 48;
      mistPhase[i] = Math.random() * Math.PI * 2;
    }
    mistGeo.setAttribute('position', new THREE.BufferAttribute(mistPos, 3));
    mistGeo.setAttribute('aPhase', new THREE.BufferAttribute(mistPhase, 1));
    ...
    const mistMat = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        void main() {
          vec3 pos = position;
          pos.y = mod(pos.y + uTime * 4.5 + aPhase * 8.0, 42.0);
          pos.x += sin(uTime * 2.0 + aPhase) * 4.5;
          pos.z += cos(uTime * 1.5 + aPhase) * 3.5;
          
          vec4 mvPos = modelViewMatrix * vec4(pos, 1.0);
          gl_PointSize = min(85.0, 170.0 / -mvPos.z);
          gl_Position = projectionMatrix * mvPos;
          vAlpha = smoothstep(42.0, 26.0, pos.y) * smoothstep(0.0, 4.0, pos.y) * 0.22;
        }
      `,
      fragmentShader: `
        varying float vAlpha;
        void main() {
          ...
          gl_FragColor = vec4(vec3(0.88, 0.94, 1.0), soft * vAlpha);
        }
      `,
      transparent: true, depthWrite: false, blending: THREE.AdditiveBlending
    });
    this._mistShader = mistMat;

...
export class World3D {
  ...
  _lights() {
    ...
    // Precision bias parameters for VSM shadows on curved terrain & thin geometry
    sun.shadow.bias = -0.0001;
    sun.shadow.normalBias = 0.02;
    sun.shadow.blurSamples = 8;
    sun.shadow.radius = 4; // VSM penumbra blur
  ...
  _composer() {
    ...
    // Ground Truth Ambient Occlusion — cinematic contact shadows (tuned for outdoor monument scale)
    const gtao = new GTAOPass(this.scene, this.camera, w, h);
    gtao.output = GTAOPass.OUTPUT.Default;
    gtao.blendIntensity = 1.0;
    if (gtao.updateGtaoMaterial) {
      // Scale AO sampling radius for large outdoor geometry (objects are meters apart)
      try { gtao.updateGtaoMaterial({ radius: 0.85, distanceExponent: 2.0 }); } catch (_) {}
    }
    ...
    const CinematicLensShader = {
      uniforms: {
        ...
        uChromaticDispersion: { value: 0.0020 },
    ...
    mat.onBeforeCompile = (shader) => {
      ...
      shader.fragmentShader = shader.fragmentShader
        ...
        .replace('#include <normal_fragment_begin>', `
          ...
          vec3 gNorm1 = texture2D(uMeadowNorm, vWorldPosition.xz * 0.045).xyz * 2.0 - 1.0;
        ...
        .replace('#include <map_fragment>', `
          ...
          vec3 grassFine   = texture2D(uMeadowMap, vWorldPosition.xz * 0.08).rgb;
          vec3 grassMid    = texture2D(uMeadowMap, vWorldPosition.xz * 0.02).rgb;
          ...
          float distToLakeEdge = length(vWorldPosition.xz - vec2(430.0, -260.0));
        ...
        .replace('#include <roughnessmap_fragment>', `
          ...
          float dToLakeRough = length(vWorldPosition.xz - vec2(430.0, -260.0));
...
//     matMossBoulder.color.setHex(0x4a5d4e);
  ...
  _animate() {
    ...
    if (this.scene.fog) {
      ...
      // Throttled shadow map update (every 60 frames) to reflect solar motion during day phases
      if (this.renderer?.shadowMap && !this.renderer.shadowMap.autoUpdate) {
        if (!this._shadowFrameCount) this._shadowFrameCount = 0;
        this._shadowFrameCount++;
        if (this._shadowFrameCount >= 60) {

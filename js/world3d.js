...
//     matMossBoulder.color.setHex(0x4a5d4e);
  ...
  _mountainWaterfall() {
    ...
    // 1. High Alpine Glacial Reservoir Tarn at (x=0, y=182, z=-570) — south of cathedral
    const upperTarnGeo = new THREE.CircleGeometry(35, 36);
    const upperTarnMesh = new THREE.Mesh(upperTarnGeo, this.waterMat);
    upperTarnMesh.rotation.x = -Math.PI / 2;
    upperTarnMesh.position.set(0, 182.0, -570);
    upperTarnMesh.receiveShadow = true;
    upperTarnMesh.frustumCulled = false;
    g.add(upperTarnMesh);

    // Full mountain waterfall: from alpine peak (z=-690, y=245) cascading down the
    // exposed cliff face, through tarn, over cliff lip, plunging to pool
    // Upper points pushed FORWARD (higher z) so water flows down the face of the rock
    const fallPoints = [
      new V3(0, 245, -690),     // Mountain peak source — on the face, not behind
      new V3(0, 228, -660),     // Upper alpine cascade — forward of mountain bulk
      new V3(0, 210, -620),     // Steep exposed cliff descent
      new V3(0, 192, -585),     // Approaching tarn
      new V3(0, 182, -570),     // Tarn surface level
      new V3(0, 180, -530),     // Tarn outflow
      new V3(0, 178, -500),     // Upper rapids
      new V3(0, 175, -460),     // Cliff lip
      new V3(0, 138, -440),     // Free fall begins
      new V3(0, 83.9, -410),    // Mid-cataract
      new V3(0, 42.0, -385),    // Lower cataract
      new V3(0, 24.0, -368),    // Near base
      new V3(0, 18.0, -360),    // Plunge pool
    ];
    const fallCurve = new THREE.CatmullRomCurve3(fallPoints);

    // 2. Dark Wet Granite Bedrock Backing — follows 2m behind waterfall ribbon
    const bedrockPoints = [
      new V3(0, 243, -692),
      new V3(0, 226, -662),
      new V3(0, 208, -622),

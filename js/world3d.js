...
export class World3D {
  ...
  _init() {
    ...
    const maxDpr = isMobileDevice ? 0.6 : (window.innerWidth > 2000 ? 0.65 : 0.75);
  ...
  _animate() {
    ...
    try {
      ...
      if (now - this._lastFpsHudUpdate >= 200 && this._fpsCount >= 5) {
        ...
        // Automatically lower pixel ratio when FPS drops below target
        if (!this._drsPixelRatio) this._drsPixelRatio = this.renderer.getPixelRatio();
        if (!this._drsMinDpr) this._drsMinDpr = 0.35;
        if (!this._drsMaxDpr) {
          const isMob = /Mobi|Android/i.test(navigator.userAgent) || window.innerWidth <= 768;
          this._drsMaxDpr = isMob ? 0.6 : 0.75;
        }
        
        if (currentFps < 45 && this._drsPixelRatio > this._drsMinDpr) {
          this._drsPixelRatio = Math.max(this._drsMinDpr, this._drsPixelRatio - 0.03);
          ...
          this._drsPixelRatio = Math.min(this._drsMaxDpr, this._drsPixelRatio + 0.01);
          this.renderer.setPixelRatio(this._drsPixelRatio);
          if (this.composer) this.composer.setPixelRatio(this._drsPixelRatio);
        }
      }
    ...
    if (this._troutMesh && this._troutData) {
      ...
      for (let i = 0; i < len; i++) {
        ...
        dummy.position.set(fx, Math.max(fy, groundH + 0.3), fz);
        dummy.rotation.set(

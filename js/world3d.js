  ...
  _animate() {
    ...
    try {
      ...
      if (now - this._lastFpsHudUpdate >= 200 && this._fpsCount >= 5) {
        ...
        // Start low and ramp UP to maintain 60fps from the first frame
        if (!this._drsPixelRatio) this._drsPixelRatio = 0.4;  // Start low, ramp up
        if (!this._drsMinDpr) this._drsMinDpr = 0.3;
        if (!this._drsMaxDpr) {

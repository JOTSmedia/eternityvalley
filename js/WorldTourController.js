...
export class WorldTourController {
  ...
  _setupWalkControls() {
    ...
    this.world.canvas.addEventListener('touchend', this.world._onTouchEnd);
  ...
  _initTourSpline() {
    ...
    const tourPoints = [
      ...
      new THREE.Vector3(360, 80.0, -180),  // 72: High coastal flight
      new THREE.Vector3(120, 52.0, 950),   // 73: Open coastal flight
      new THREE.Vector3(50, 48.0, 1950),   // 74: Approach Kaya Island
      new THREE.Vector3(80, 48.0, 2100),   // 75: Eastward orbit around Kaya Statue Orb (x:20, z:2100)
      new THREE.Vector3(20, 48.0, 2160),   // 76: Southward orbit around Kaya Statue Orb
      new THREE.Vector3(-40, 48.0, 2100),  // 77: Westward orbit completion
      new THREE.Vector3(-110, 34.0, 2160), // 78: Close pass by Coastal Cliff Building (Temple of Baal, x:-110, z:2160)
      new THREE.Vector3(-45, -14.0, 2270), // 79: Oceanic coral abyss plunge
  ...
  _updateWalk(dt) {
    ...
    if (this.tourMode) {
      ...
      // Quaternion Slerp for Cinematic Camera Rotation (Prevents Gimbal Lock & Lerp Snap)
      // 1. Calculate ideal target rotation quaternion
      _mat4Temp.lookAt(this.world._v3TourPos, this.world._v3TourLook, new THREE.Vector3(0, 1, 0));
      const targetQuat = new THREE.Quaternion().setFromRotationMatrix(_mat4Temp);
      ...
      // Apply roll in local space (Z-axis)
      const rollQuat = new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0, 0, 1), this.world._currentRoll);
      targetQuat.multiply(rollQuat); // Local rotation

      // Initialize current quaternion if missing
      if (!this.world._currentQuat) {
        this.world._currentQuat = this.world.camera.quaternion.clone();
      }

      // Smooth spherical interpolation (Slerp) towards target rotation
      const lookDamp = 1.0 - Math.exp(-8.8 * safeDt);
      this.world._currentQuat.slerp(targetQuat, lookDamp);

      this.world.camera.position.copy(this.world._v3TourPos);
      this.world.camera.quaternion.copy(this.world._currentQuat);

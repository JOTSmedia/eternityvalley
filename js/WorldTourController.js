...
export class WorldTourController {
  ...
  _initTourSpline() {
    ...
    const tourPoints = [
      ...
      new THREE.Vector3(0, 36.5, 885),  // 36.5m perfectly clears the 56m crossbeam
      ...
      // 6. CATHEDRAL - Flying straight between open French Gothic oak doors
      new THREE.Vector3(0, 188.0, -660),  // Straight flight towards front doors
      new THREE.Vector3(0, 195.0, -740),  // Penetrating the nave aisle

      // 7. MOSQUE - Gliding over the marble reflecting pool and Alhambra arcade
      new THREE.Vector3(-350, 160.0, -350), 
      new THREE.Vector3(-480, 104.0, -150), // Gliding intimately over 96m terrace

      // 8. LAKE - Skimming lake waters, diving underwater with koi
      ...
      // 9. PAGODA - Flying past South Porch, spiraling up past eaves
      ...
      // 10. KAYA ISLAND - 360-degree rotation, gliding past Cliff Temple, plunging
      new THREE.Vector3(300, 280.0, 800),  // Soaring over the 245m mountain ridge
      new THREE.Vector3(80, 60.0, 2150),   // Gliding past Cliff Temple of Baal (y=22m), plunging

      // 11. PANORAMA - Radiant sunrise ocean breach for panoramic climb
      new THREE.Vector3(-100, 8.0, 1500),  // Ocean breach from underwater
      new THREE.Vector3(0, 60.0, 1300), // Loops back to Gate
    ];
  ...
  _calculateTourLookTarget(t, pos, tangent, outTarget) {
    ...
    // This allows the camera to comfortably frame the subject before ripping away
    ...
    // Stage 11 is panorama climb (look ahead).
    // All other stages are orbits/dives/glides that demand strong POI tracking.
    const isStraightApproach = (stageIdx === 0 || stageIdx === 1 || stageIdx === 10);

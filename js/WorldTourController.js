...
export class WorldTourController {
  ...
  _initTourSpline() {
    ...
    const tourPoints = [
      ...
      new THREE.Vector3(0, 55.0, -360),   // 28: Approaching the grand gorge
      new THREE.Vector3(0, 45.0, -420),  // 29: Entering the sheer basalt canyon
      new THREE.Vector3(0, 30.0, -480),  // 30: Skimming just above the river rapids
      new THREE.Vector3(0, 15.0, -520),  // 31: Hydrodynamic dive into plunge basin!

      // === LEG 5: HIGHLAND WATER SOURCE & SUBMERGED TARN DIVE (Stage 5: t in [0.364, 0.455], indices 32..39) ===
      new THREE.Vector3(0, 10.0, -540),  // 32: Submerged dive into roaring plunge basin
      new THREE.Vector3(12, 12.0, -550), // 33: Deep submerged glide near waterfall impact
      new THREE.Vector3(14, 18.0, -560), // 34: Swimming up towards the falls
      new THREE.Vector3(0, 35.0, -570),  // 35: Bursting out of the water!
      new THREE.Vector3(0, 85.0, -580),  // 36: Mid-cataract vertical rocket climb!
      new THREE.Vector3(0, 150.0, -590), // 37: Soaring vertically up the massive 180m cliff
      new THREE.Vector3(0, 230.0, -600), // 38: Cresting the lip of Eternity Falls into an aerial climb!
      new THREE.Vector3(45, 255.0, -620), // 39: Soaring high into Cathedral aerial orbit approach!


      // === LEG 6: UNIVERSAL CATHEDRAL AERIAL ORBIT (Stage 6: t in [0.455, 0.545], indices 40..47) ===
      new THREE.Vector3(90, 260.0, -650), // 40: High East aerial orbit
      new THREE.Vector3(0, 270.0, -750),  // 41: High North aerial orbit commanding panoramic vista
      new THREE.Vector3(-90, 260.0, -650),// 42: High West aerial orbit
      new THREE.Vector3(0, 215.0, -580),  // 43: Descending ceremonial avenue
      new THREE.Vector3(0, 192.0, -595),  // 44: Approaching West Portal steps
      new THREE.Vector3(0, 190.0, -610),  // 45: Entering straight between open doors
      new THREE.Vector3(-15, 190.0, -645),// 46: Banking left inside transept crossing
      new THREE.Vector3(-60, 192.0, -654),// 47: Flying out North Transept archway smoothly
  ...
  _calculateTourLookTarget(t, pos, tangent, outTarget) {
    ...
    if (normT < 4 * S) {
      if (normT < 3 * S + S * 0.4) {
        target.set(0.0, 65.0, -360.0);
      } else if (normT < 3 * S + S * 0.8) {
        target.set(0.0, Math.min(215.0, pos.y + 35.0), -450.0);
      } else {
        const frac = smooth((normT - (3 * S + S * 0.8)) / (S * 0.2));
        target.set(0.0, 184.0 + (177.5 - 184.0) * frac, -460.0 + (-500.0 - (-460.0)) * frac);
      }
      return target;
    }
    ...
    if (normT < 5 * S) {
      if (normT < 4 * S + S * 0.3) {
        ...
        target.set(0.0, 172.0 + (169.0 - 172.0) * frac, -500.0 + (-525.0 - (-500.0)) * frac);
      } else if (normT < 4 * S + S * 0.7) {
        const frac = smooth((normT - (4 * S + S * 0.3)) / (S * 0.4));
        target.set(12.0 + (0.0 - 12.0) * frac, 169.0 + (174.0 - 169.0) * frac, -525.0 + (-550.0 - (-525.0)) * frac);
      } else {
        const frac = smooth((normT - (4 * S + S * 0.7)) / (S * 0.3));
        target.set(0.0, 174.0 + (235.0 - 174.0) * frac, -550.0 + (-650.0 - (-550.0)) * frac);
      }
      return target;
    }
    ...
    if (normT < 6 * S) {
      if (normT < 5 * S + S * 0.4) {
        target.set(0.0, 215.0, -650.0);
      } else if (normT < 5 * S + S * 0.6) {
        const frac = smooth((normT - (5 * S + S * 0.4)) / (S * 0.2));
        target.set(0.0, 215.0 + (196.0 - 215.0) * frac, -650.0 + (-610.0 - (-650.0)) * frac);
      } else if (normT < 5 * S + S * 0.8) {
        target.set(0.0, 188.5, -685.0);
      } else if (normT < 5 * S + S * 0.9) {
        const frac = smooth((normT - (5 * S + S * 0.8)) / (S * 0.1));
        target.set(0.0 + (-60.0 - 0.0) * frac, 188.5 + (190.0 - 188.5) * frac, -685.0 + (-654.0 - (-685.0)) * frac);
      } else {

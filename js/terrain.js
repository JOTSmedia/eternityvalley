...
export function terrainHeightBase(x, z) {
  ...
  if (z < -340) {
    ...
    // Wider notch (65m from center = 130m opening) so waterfall is visible from all angles
    const notchMask = sstep(65, 280, Math.abs(x));
  ...
  // Carves from tarn outflow (z=-500) through cliff face to plunge pool (z=-340)
  if (Math.abs(x) < 55 && z <= -340 && z >= -500) {
    const gorgeMask = (1.0 - Math.abs(x) / 55);
    let gorgeSlope;
    if (z <= -460) {
      // Tarn outflow to cliff lip (175)
      const t = sstep(-500, -460, z);
      gorgeSlope = lerp(180.0, 175.0, t);
    } else {
      // Main waterfall: cliff lip (175) to plunge pool (16)
      const gorgeDrop = sstep(-340, -460, z);
      gorgeSlope = lerp(16.0, 175.0, gorgeDrop);
    }
    h = Math.min(h, lerp(h, gorgeSlope - 6.0, gorgeMask));
  }

...
export function terrainHeightBase(x, z) {
  ...
  if (dl < WORLD.lake.r + 35) {
    const lakeBedTarget = -25.0; // 37.5m deep crystal clear lake basin beneath 12.5m water surface
  ...
  if (dPlunge < 72.0) {
    const poolBedTarget = -15.0; // 33.0m deep churning plunge basin beneath 18.0m water surface
  ...
  if (z <= -450 && z >= -820 && Math.abs(x) < 260) {
    ...
    const targetH = lerp(184.5 + roll * (1.0 - catTerrace * 0.98), 120.0, waterCarve * 0.98);

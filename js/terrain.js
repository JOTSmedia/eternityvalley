...
export function terrainHeightBase(x, z) {
  ...
  if (riverInfo.dist < 45.0 && z > -360 && z < 915) {
    ...
    if (riverInfo.dist < channelWidth) {
      ...
      const targetRiverBed = riverInfo.y - 32.0; // 32.0m deep natural river canyon
  ...
  if (dPlunge < 72.0) {
    const poolBedTarget = -81.0; // 33.0m deep churning plunge basin beneath 18.0m water surface
  ...
  if (z <= -450 && z >= -820 && Math.abs(x) < 260) {
    ...
    const targetH = lerp(184.5 + roll * (1.0 - catTerrace * 0.98), -4.0, waterCarve * 0.98);
...
const _hCache = new Float32Array(201 * 201);
const _hCacheValid = new Uint8Array(201 * 201);

function _getCachedHeight(ix, iz, x, z) {
  const key = iz * 201 + ix;
  if (_hCacheValid[key] === 1) return _hCache[key];
...
export function terrainHeight(x, z) {
  ...
  const h00 = _getCachedHeight(ix, iz, x0, z0);
  const h10 = _getCachedHeight(ix + 1, iz, x1, z0);
  const h01 = _getCachedHeight(ix, iz + 1, x0, z1);
  const h11 = _getCachedHeight(ix + 1, iz + 1, x1, z1);

...
export function terrainHeightBase(x, z) {
  ...
  // Clear notch and gorge channel directly along the waterfall cataract line (x ≈ 0, z = -340..-660)
  if (Math.abs(x) < 50 && z <= -340 && z >= -660) {
    const gorgeMask = (1.0 - Math.abs(x) / 50);
    let gorgeSlope;
    if (z <= -640) {
      // Tarn lip
      gorgeSlope = 181.0;
    } else if (z <= -460) {
      // Upper cliff face: steep descent from tarn (182) to cliff lip (175)
      const upperT = sstep(-640, -460, z);
      gorgeSlope = lerp(181.0, 175.0, upperT);
    } else {
      // Main waterfall: dramatic drop from cliff lip (175) down to plunge pool (16)
      const gorgeDrop = sstep(-340, -460, z);
      gorgeSlope = lerp(16.0, 175.0, gorgeDrop);
    }
    h = Math.min(h, lerp(h, gorgeSlope - 4.0, gorgeMask));
  }

...
export function terrainHeightBase(x, z) {
  ...
  // Highland Glacial Tarn Water Source Basin at (x=0, z=-520, waterLevel=182.0m)
  // South of the Universal Cathedral (z=-640), directly feeding the cataract waterfall (z=-460)
  const dxTarn = x / 36.0;
  const dzTarn = (z - (-520)) / 34.0;
  const dTarnNorm = Math.hypot(dxTarn, dzTarn);
  if (dTarnNorm < 1.35) {
    const tarnBedTarget = 173.0; // 9.0m deep crystal alpine lake bed beneath 182.0m water surface
  ...
  if (z <= -450 && z >= -820 && Math.abs(x) < 260) {
    ...
    const tarnHole = Math.exp(-Math.pow(dTarnNorm / 1.3, 2.0));
    // channel from tarn outlet to cliff lip
    const chanFactor = (z >= -520 && z <= -460) ? Math.exp(-Math.pow(x/25.0, 2.0)) : 0.0;
  ...
  if (Math.abs(x) < 72 && z <= -340 && z >= -510) {
    ...
    if (z <= -460) {
      ...
      gorgeBedY = lerp(173.0, 168.0, t);
    } else {
      const u = clamp((-358 - z) / 98.0, 0.0, 1.0);
      gorgeBedY = 13.5 + Math.pow(u, 1.25) * 154.5 - 4.0;
    }
  ...
  if (z > 1700) {
    ...
    // Submerged Kaya Coral Reef Lagoon & Deep Marine Trench (z = 2160..2520, |x| < 320)
    // Deep crystal ocean lagoon (bedrock carved down to y = -18.5m, allowing deep pelagic exploration)
    if (z >= 2160 && z <= 2520 && Math.abs(x) < 320) {
      const dropT = sstep(2160, 2220, z);
      const reefLagoonMask = Math.cos(Math.min(1.0, Math.hypot(x / 240, (z - 2340) / 140)) * Math.PI * 0.5);
      const targetBed = -18.5; // Deep ocean trench

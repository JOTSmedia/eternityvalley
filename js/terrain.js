...
export function terrainHeightBase(x, z) {
  ...
  if (z < -340) {
    // Headwall slope behind cataract from y=14.0m plunge basin at z=-350 up to y=182.0m plateau at z=-460
    const headwallDrop = sstep(-350, -460, z);
    ...
    const cirqueElevation = 24.0 + sstep(-340, -740, z) * 215.0 + ridgeNoise;
  ...
  if (z > 915) {
    ...
    const seaFloor = -42.0 * (1.0 - coveMask * 0.85); // 42m deep ocean seabed
  ...
  if (riverInfo.dist < 45.0 && z > -360 && z < 915) {
    ...
    if (riverInfo.dist < channelWidth) {
      ...
      const targetRiverBed = riverInfo.y - 4.5; // 4.5m deep natural river canyon
  ...
  if (dl < WORLD.lake.r + 35) {
    const lakeBedTarget = -2.0; // 14.5m deep crystal clear lake basin beneath 12.5m water surface
    const shoreBlend = sstep(WORLD.lake.r - 35, WORLD.lake.r + 35, dl);
    h = lerp(lakeBedTarget, h, shoreBlend);
  }

  // Highland Glacial Tarn Water Source Basin at (x=0, z=-515, waterLevel=182.0m)
  // Deep alpine reservoir pool south of the Universal Cathedral (z=-640), feeding cataract waterfall (z=-460)
  // Covering z = -460..-570, |x| < 60 with 24.0m crystal clear water depth (bed at y = 158.0m)
  if (z >= -575 && z <= -455 && Math.abs(x) < 70) {
    const tarnXNorm = Math.abs(x) / 54.0;
    const tarnZDist = (z > -515) ? (z - (-515)) / 55.0 : ((-515) - z) / 55.0;
    const dTarnNorm = Math.hypot(tarnXNorm, tarnZDist);
    if (dTarnNorm < 1.35) {
      const tarnBedTarget = 158.0; // 24.0m deep crystal alpine pool beneath 182.0m water surface
      const tarnShoreBlend = sstep(0.75, 1.35, dTarnNorm);
  ...
  if (dPlunge < 72.0) {
    const poolBedTarget = 4.5; // 13.5m deep churning plunge basin beneath 18.0m water surface
  ...
  if (z <= -450 && z >= -820 && Math.abs(x) < 260) {
    ...
    // Tarn basin carving in plateau
    const tarnXNorm = Math.abs(x) / 54.0;
    const tarnZDist = (z > -515) ? (z - (-515)) / 55.0 : ((-515) - z) / 55.0;
    const dTarnNorm = Math.hypot(tarnXNorm, tarnZDist);
    const tarnHole = Math.exp(-Math.pow(dTarnNorm / 0.95, 2.0));

    // Channel from tarn outlet to cliff lip
    const chanFactor = (z >= -520 && z <= -455 && Math.abs(x) < 32.0) ? Math.exp(-Math.pow(x / 24.0, 2.0)) : 0.0;
    const waterCarve = Math.max(tarnHole, chanFactor);
    const targetH = lerp(184.5 + roll * (1.0 - catTerrace * 0.98), 158.0, waterCarve * 0.98);
    h = lerp(h, targetH, plateauEdge * 0.98);
  }

  // Waterfall gorge channel — MUST run AFTER plateau carve to cut through it
  // Carves a deep, wide parabolic couloir from tarn outflow (z=-510) down to plunge pool (z=-340)
  if (Math.abs(x) < 72 && z <= -340 && z >= -510) {
    const gorgeMask = sstep(72.0, 26.0, Math.abs(x));
    let gorgeBedY;
    if (z <= -460) {
      const t = clamp((z - (-510)) / 50.0, 0.0, 1.0);
      gorgeBedY = lerp(158.0, 154.0, t);
    } else {
      const u = clamp((-358 - z) / 98.0, 0.0, 1.0);
  ...
  if (z > 1700) {
    ...
    if (z >= 2100 && z <= 2750 && Math.abs(x) < 450) {
      ...
      const targetBed = -42.0; // True 42m deep ocean abyss
...
export function isUnderWater(x, z) {
  ...
  if (z <= -455 && z >= -575 && Math.abs(x) <= 65) waterY = 182.0;
  else if (Math.hypot(x, z - (-360)) < 70) waterY = 18.0;
  else if (z > 950) waterY = WORLD.oceanLevel || 0.35;
  else {
    const rInfo = getRiverInfo(x, z);
    if (rInfo.dist < 36.0 && z > -360 && z <= 950) waterY = rInfo.y;
  }

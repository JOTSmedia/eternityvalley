...
export function terrainHeightBase(x, z) {
  ...
  if (z < -540) {
    ...
    const headwallDrop = sstep(-550, -610, z);
    ...
    const headwallTerrain = (z <= -610) ? plateau : headwallElevation;
  ...
  if (z <= -610 && z >= -820 && Math.abs(x) < 260) {
    const plateauEdge = (1.0 - sstep(160, 260, Math.abs(x))) * (1.0 - sstep(-890, -970, z));
...
export function isUnderWater(x, z) {
  ...
  if (z <= -615 && z >= -735 && Math.abs(x) <= 65) waterY = 182.0;

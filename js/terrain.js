...
export function terrainHeightBase(x, z) {
  ...
  // Highland Glacial Tarn Water Source Basin at (x=0, z=-570, waterLevel=182.0m)
  // South of the Universal Cathedral, feeding the waterfall — must NOT overlap cathedral footprint at z=-640
  const dxTarn = x / 30.0;
  const dzTarn = (z - (-570)) / 28.0;
  ...
  if (z <= -450 && z >= -820 && Math.abs(x) < 260) {
    ...
    const chanFactor = (z >= -695 && z <= -460) ? Math.exp(-Math.pow(x/25.0, 2.0)) : 0.0;
  ...
  // Carves from mountain face (z=-695) through tarn, cliff face, to plunge pool (z=-340)
  if (Math.abs(x) < 60 && z <= -340 && z >= -695) {
    const gorgeMask = (1.0 - Math.abs(x) / 60);
    let gorgeSlope;
    if (z <= -690) {
      gorgeSlope = 243.0;
    } else if (z <= -620) {
      // Mountain face descent: peak (245) to steep cliff (210)
      const t = sstep(-690, -620, z);
      gorgeSlope = lerp(243.0, 208.0, t);
    } else if (z <= -570) {
      // Cliff to tarn (182)
      const t = sstep(-620, -570, z);
      gorgeSlope = lerp(208.0, 180.0, t);
    } else if (z <= -460) {
      // Tarn outflow to cliff lip (175)
      const t = sstep(-570, -460, z);
      gorgeSlope = lerp(180.0, 173.0, t);
    } else {
      // Main waterfall: cliff lip (175) to plunge pool (16)
      const gorgeDrop = sstep(-340, -460, z);
      gorgeSlope = lerp(16.0, 173.0, gorgeDrop);
    }
    h = Math.min(h, gorgeSlope - 8.0);
  }

...
export function terrainHeightBase(x, z) {
  ...
  // 1. Jagged Alpine Glacial Mountain Cirque (Encompassing the sanctuary on North, East, and West flanks)
  const sharpArête = Math.pow(ridgeNoise(x * 0.0035, z * 0.0035, 6), 2.2);
  const secondaryRidge = ridgeNoise(x * 0.0070 + 4.2, z * 0.0070 + 4.2, 4);
  const couloirErosion = Math.pow(Math.abs(fbm(x * 0.0080 + 3.1, z * 0.0080 + 3.1, 4) - 0.5) * 2.0, 2.0);
  const peakMacro = fbm(x * 0.0018 + 12.0, z * 0.0018 + 12.0, 4);
  
  // Geological rock horns (sharp alpine spires surrounding the valley)
  const hornWest = Math.exp(-Math.pow(dist(x, z, -680, -320) / 180, 2)) * 140;
  const hornEast = Math.exp(-Math.pow(dist(x, z, 640, -380) / 180, 2)) * 130;
  const hornNorthW = Math.exp(-Math.pow(dist(x, z, -180, -580) / 160, 2)) * 110;
  const hornNorthE = Math.exp(-Math.pow(dist(x, z, 180, -580) / 160, 2)) * 110;
  const horns = hornWest + hornEast + hornNorthW + hornNorthE;

  const hydraulicErosion = 0.40 + 0.85 * sharpArête + 0.35 * secondaryRidge + 0.25 * peakMacro - 0.45 * couloirErosion;
  
  // Grand surrounding jagged peaks (rising to 210m-310m high crests)
  let mountains = sstep(540, 1200, r) * (210 + peakMacro * 150) * Math.max(0, hydraulicErosion) + horns;
  ...
  if (z <= -450 && z >= -820 && Math.abs(x) < 260) {
    ...
    const tarnHole = Math.exp(-Math.pow(dTarnNorm / 1.0, 2.0));

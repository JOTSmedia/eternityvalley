...
const RECIPES = {
  ...
  /** Mountain rock cliff face with sheer basalt ridges, deep crevices, and stark rugged contrast. */
  rockCliff(size = 256) {
    const seed = 419;
    const h = heightField(size, (x, y, u, v) => {
      // Rugged columnar basalt cliff face
      const sheerCrags = fbmTile(u * 6, v * 24, 12, 3, seed) * 0.65; // stretched vertically for sheer drops
      const fractures = Math.abs(fbmTile(u * 24 + 1.2, v * 12 + 2.3, 16, 3, seed + 7) - 0.5) * 0.75;
      const micro = fbmTile(u * 64, v * 64, 48, 3, seed + 13) * 0.22;
      return sheerCrags + fractures + micro;
    });
    const map = paint(size, (x, y, u, v) => {
      const hv = h[y * size + x];
      // Basalt is dark grey to black, with some iron oxidation/rust and damp moss
      const rust = Math.pow(fbmTile(u * 12, v * 12, 12, 3, seed + 29), 3.5) * 0.45;
      const dampMoss = Math.pow(fbmTile(u * 16, v * 8, 16, 3, seed + 31), 3.0) * 0.3;
      const basalt = [42, 44, 46]; // dark charcoal basalt
      const c = tint(basalt, 0.6 + hv * 0.5);
      return [
        clamp255(c[0] + rust * 55 - dampMoss * 10),
        clamp255(c[1] + rust * 25 + dampMoss * 30),
        clamp255(c[2] + rust * 15 - dampMoss * 10),
      ];
    });
    const rough = paint(size, (x, y, u, v) => {
      // Wet sheen on the rock face near waterfalls
      const wetSheen = Math.pow(fbmTile(u * 8, v * 8, 8, 3, seed + 41), 2.0) * 120;
      const g = clamp255(140 + fbmTile(u * 32, v * 32, 28, 3, seed + 3) * 65 - wetSheen);
      return [g, g, g];
    });
    const ao = paint(size, (x, y) => {
    ...
    return { map, normal: normalFromHeight(size, h, 3.5), rough, ao }; // deeper normal for ruggedness
  },
  ...
  /** Cathedral Basalt Ashlar — Rugged, sheer drops and majestic dark stone with high contrast AO */
  agedCaenLimestone(size = 512) {
    ...
    const h = heightField(size, (x, y, u, v) => {
      ...
      const grain = fbmTile(u * 12, v * 12, 12, 3, seed) * 0.25;
      const pits = Math.pow(fbmTile(u * 28, v * 28, 28, 2, seed + 5), 4.5) * 0.25;
      const sheerFractures = Math.abs(fbmTile(u * 64, v * 12, 64, 3, seed + 11) - 0.5) * 0.15;
      return jointDepth * (0.82 + grain + sheerFractures) - pits;
    });
    const map = paint(size, (x, y, u, v) => {
      const hv = h[y * size + x];
      const grain = fbmTile(u * 14, v * 14, 14, 3, seed + 2);
      // Dark rugged basalt
      const darkBasalt = [48, 52, 56];
      const darkMortar = [24, 26, 28];
      let r = darkBasalt[0] * (0.85 + hv * 0.15 + grain * 0.05);
      let g = darkBasalt[1] * (0.85 + hv * 0.15 + grain * 0.05);
      let b = darkBasalt[2] * (0.85 + hv * 0.15 + grain * 0.05);
      if (hv < 0.88) {
        const k = (0.88 - hv) / 0.15;
        const clampedK = Math.min(1.0, Math.max(0.0, k));
        r = r * (1 - clampedK) + darkMortar[0] * clampedK;
        g = g * (1 - clampedK) + darkMortar[1] * clampedK;
        b = b * (1 - clampedK) + darkMortar[2] * clampedK;
      }
      return [clamp255(r), clamp255(g), clamp255(b)];
    });
    const rough = paint(size, (x, y, u, v) => {
      const hv = h[y * size + x];
      const g = clamp255(165 - hv * 20 + fbmTile(u * 24, v * 24, 24, 2, seed + 3) * 35);
      return [g, g, g];
    });
    const ao = paint(size, (x, y) => {
      const joint = mortar(x / size, y / size);
      const jointAO = smooth(Math.min(1.0, joint / 0.012));
      const g = clamp255(80 + jointAO * 110 + h[y * size + x] * 65);
      return [g, g, g];
    });
    return { map, normal: normalFromHeight(size, h, 2.5), rough, ao };
  },
...
export function material(name, opts = {}) {
  ...
  const Ctor = THREE.MeshPhysicalMaterial; // Forced UE5-level physical material
  const mat = new Ctor({
    map: cloneWith(t.map),
    normalMap: cloneWith(t.normalMap),
    roughnessMap: cloneWith(t.roughnessMap),
    aoMap: cloneWith(t.aoMap),
    envMapIntensity: opts.envMapIntensity || 1.2, // Boost environmental reflections
    ...rest,
  });
  if (mat.normalMap) {
...
export const Surfaces = {
  ...
  byzantineMosaic: (repeat = 1, o = {}) => material('byzantineMosaic', { repeat, color: 0xffffff, roughness: 0.25, metalness: 0.65, physical: true, clearcoat: 0.85, clearcoatRoughness: 0.15, normalScale: 0.85, envMapIntensity: 1.6, ...o }),
  ...
  flagstone: (repeat = 3, o = {}) => material('limestone', {  repeat, color: 0xb2a58d, roughness: 0.84, metalness: 0.02, normalScale: 1.3, envMapIntensity: 1.05, polygonOffset: true, polygonOffsetFactor: -4.0, polygonOffsetUnits: -4.0, ...o }),
  ...
  photogrammetryRock: (repeat = 8, o = {}) => material('photogrammetryRock', {  repeat, color: 0xffffff, roughness: 0.84, metalness: 0.03, normalScale: 3.8, aoMapIntensity: 2.2, envMapIntensity: 0.98 , ...o }),
  stainedGlassRose: () => {

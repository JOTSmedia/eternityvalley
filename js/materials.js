...
export function material(name, opts = {}) {
  ...
  const Ctor = THREE.MeshStandardMaterial;
  const mat = new Ctor({
    map: cloneWith(t.map),
...
export const Surfaces = {
  ...
  water: (repeat = 1, o = {}) => new THREE.MeshPhysicalMaterial({
    color: 0xffffff,
    transmission: 1.0,
    ior: 1.333,
    roughness: 0.10,
  ...
  glass: () => new THREE.MeshStandardMaterial({
    color: 0xd0f0ff, roughness: 0.03, metalness: 0.1, transparent: true, opacity: 0.65, envMapIntensity: 2.2
  }),
  crystal: () => new THREE.MeshStandardMaterial({
    color: 0x38bdf8, roughness: 0.02, metalness: 0.1, transparent: true, opacity: 0.80, envMapIntensity: 2.4
  }),
  prismaticCrystal: () => new THREE.MeshStandardMaterial({
    color: 0xffffff, roughness: 0.02, metalness: 0.15, transparent: true, opacity: 0.82, envMapIntensity: 2.5
  }),
  ...
  memoryCrystal: (color = 0x78dcfa) => new THREE.MeshStandardMaterial({
    color: 0xffffff, emissive: new THREE.Color(color), emissiveIntensity: 1.6, roughness: 0.02, metalness: 0.1, transparent: true, opacity: 0.85, envMapIntensity: 2.2
  }),
  ...
  starlightCrystal: () => new THREE.MeshStandardMaterial({
    color: 0x90e0ef, roughness: 0.02, metalness: 0.15, transparent: true, opacity: 0.85, envMapIntensity: 2.5
  }),
  crystalColumn: () => new THREE.MeshStandardMaterial({
    color: 0xf0faff, roughness: 0.02, metalness: 0.1, transparent: true, opacity: 0.80, envMapIntensity: 2.2
  }),
  ...
  mat.onBeforeCompile = (shader) => {
    try {
      ...
      shader.vertexShader = shader.vertexShader.replace(
        ...
        // Multi-frequency organic wind gust displacement
        float lowFreq = (sin(branchPhase) * 0.38 + cos(branchPhase * 0.72 + wPos.y * 0.04) * 0.20);
        float midFreq = (cos(branchPhase * 2.3 + position.y * 0.30) * 0.16);
        float highFreq = (sin(branchPhase * 5.4 + position.x * 1.8 + position.z * 1.8) * 0.07);
        float gust = (lowFreq + midFreq + highFreq) * uWindIntensity;

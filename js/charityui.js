  ...
  donateModal(ui, c) {
    ...
    const paint = () => {
      ...
      box.querySelector('#giveBreak').innerHTML = `
        <div class="give-break__row"><span>Your donation</span><b>${fmt(s.gross)}</b></div>
        <div class="give-break__row is-fee"><span>Direct processing (Stripe rate)</span><b>−${fmt(s.processor)}</b></div>
        <div class="give-break__row is-fee"><span>Platform fee</span><b>${fmt(0)} (0%)</b></div>
        <div class="give-break__row is-total"><span>Delivered to ${esc(ch?.name || 'the charity')}</span><b>${fmt(s.charity)}</b></div>`;
    };
    paint();

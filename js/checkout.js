...
export async function checkout({ kind, name, amount, meta = {} }) {
  ...
  if (numAmount < 0.50 && numAmount > 0) {
    return { ok: false, error: 'Minimum transaction is $0.50' };
  }
  ...
  if (!HAS_API && !IS_DEMO) {
    return { ok: false, error: 'Payment network unavailable in production.' };
  }
  let res;
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      if (attempt === 1) {
        return { ok: false, error: 'Network failed to reach payment provider.' };
      }
      await new Promise(r => setTimeout(r, 600));
    }
  }
  if (!res || !res.ok) {
    return { ok: false, error: 'Payment server rejected the request.' };
  }

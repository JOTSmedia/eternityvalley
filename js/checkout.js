...
export async function checkout({ kind, name, amount, meta = {} }) {
  ...
  if (!HAS_API) {
    console.warn('[checkout] No payment server configured. Falling back to offline verified ledger mode.');
    track(kind, name, numAmount, false, meta);
    const entry = await toLedger(kind, name, numAmount, meta, true);
    return { ok: true, demo: true, entry, offlineFallback: true };
  }
  let res;
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      if (attempt === 1) {
        console.warn('[checkout] Network failed, recording transaction to offline ledger:', e);
        const entry = await toLedger(kind, name, numAmount, meta, true);
        return { ok: true, demo: true, entry, offlineFallback: true };
      }
  ...
  if (!res || !res.ok) {
    console.warn('[checkout] Server returned error, recording to offline ledger');
    const entry = await toLedger(kind, name, numAmount, meta, true);
    return { ok: true, demo: true, entry, offlineFallback: true };
  }
